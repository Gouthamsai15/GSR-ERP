...
export const PrincipalDashboardPage = () => {
  ...
  const [examSubjectsByExam, setExamSubjectsByExam] = useState<Record<string, string>>({});
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        setExamSubjectsByExam(Object.fromEntries(subjectEntries));
        setLeaves(leaveRows);
  ...
  const performanceByExam = useMemo(() => {
    ...
    return Array.from(map.entries()).map(([label, values]) => {
      const sourceExam = results.find((result) => (result.examName ?? "Unknown") === label);
      return {
        label,
        value: Math.round(average(values)),
        subjectNames: sourceExam ? examSubjectsByExam[sourceExam.examId] ?? "Not available" : "Not available",
      };
    });
  }, [examSubjectsByExam, results]);
            ...
            {loading ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={performanceByExam}>
                  ...
                  <Tooltip
                    formatter={(value, _name, payload) => `${value}%`}
                    labelFormatter={(label, payload) => {
                      const subjectNames = payload?.[0]?.payload?.subjectNames;
                      return subjectNames ? `${label} • ${subjectNames}` : String(label);
                    }}
                  />
  ...
  return (
    <div className="space-y-6">
      ...
      <DataTable
        ...
        renderActions={(item) => (
          <Link to={`/principal/students/${item.id}`}>
  ...
  return (
    <div className="space-y-6">
      ...
      <DataTable
        ...
        renderActions={(item) => (
          <Link to={`/principal/staff/${item.id}`}>
            <Button variant="outline">View</Button>
          </Link>
        )}
        pageSize={10}
...
export const PrincipalApprovalsPage = () => {
  ...
  const [approvedApplicant, setApprovedApplicant] = useState<ApplicantRecord | null>(null);
  ...
  const handleApplicantApproval = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setApprovedApplicant({ ...approvalTarget, status: "Approved" });
      await loadData();
    } catch (submissionError) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <CompactMetricCard label="Pending Leaves" value={pendingLeaves.length} detail="Leadership-stage leave decisions waiting for the principal." icon={<span className="text-sm font-bold">LV</span>} />
      ...
      {approvedApplicant ? (
        <Card className="border-blue-200 bg-blue-50 shadow-sm">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-blue-700">Admission Approved</p>
              <p className="mt-1 text-sm text-blue-900">
                {approvedApplicant.name} is approved. Open the student details form to finish admission.
              </p>
            </div>
            <Button variant="outline" className="bg-white text-blue-700 hover:bg-blue-100" onClick={() => openStudentForm(approvedApplicant)}>
              Add to School
            </Button>
          </div>
        </Card>
      ...
      <DataTable
        ...
        description="Approve or reject leave requests after HR has forwarded them to leadership."
        data={pendingLeaves}
        getRowId={(item) => item.id}
        loading={loading}
      ...
      <DataTable
        ...
        description="Approve applicants, then finish their full student details in the student entry form."
        ...
        renderActions={(item) => (
          <div className="flex items-center gap-2">
            <Button variant="outline" onClick={() => { setApprovalTarget(item); }}>
              Approve
            </Button>
      ...
      <Modal open={Boolean(approvalTarget)} onClose={() => setApprovalTarget(null)} title="Approve Applicant" description="Approve the applicant, then continue in the student details form.">
        <form className="space-y-4" onSubmit={handleApplicantApproval}>
          <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600">
            Approval will open the student form with the applicant name and class already filled in.
          </div>
          <div className="flex justify-end gap-3">
