...
const TimetableShell = ({ mode }: { mode: TimetableMode }) => {
  ...
  const getValidBreakStartTimes = (day: TimetableDay) => {
    const daySlots = slots.filter((slot) => slot.day === day);
    return timeOptions.slice(0, -1).filter((time) =>
      daySlots.every((slot) => time <= slot.startTime || time >= slot.endTime),
    );
  };
  const getValidWeeklyBreakStartTimes = () => {
    const weekdayStartSets = DAYS.filter((day) => day !== "Sun").map(
      (day) => new Set(getValidBreakStartTimes(day)),
    );

    return timeOptions.slice(0, -1).filter((time) => weekdayStartSets.every((set) => set.has(time)));
  };
  const availableBreakStartTimeOptions = useMemo(
    () => {
      if (!form.isBreak) {
        return timeOptions.slice(0, -1);
      }

      const isSelectedClassContext = form.className === selectedClass && form.section === selectedSection;
      return isSelectedClassContext ? getValidWeeklyBreakStartTimes() : getValidBreakStartTimes(form.day);
    },
    [form.className, form.day, form.isBreak, form.section, selectedClass, selectedSection, slots, timeOptions],
  );
  ...
  const handleChange = <K extends keyof TimetableFormValues>(key: K, value: TimetableFormValues[K]) => {
    ...
    setForm((current) => {
      ...
      if (key === "isBreak") {
        if (value) {
          const validStartTimes =
            current.className === selectedClass && current.section === selectedSection
              ? getValidWeeklyBreakStartTimes()
              : getValidBreakStartTimes(current.day);
          next.subjectId = "";
          next.teacherId = "";
          next.breakType = current.breakType || "Short Break";
          next.breakLabel = current.breakLabel || current.breakType || "Short Break";
          next.startTime = validStartTimes.includes(current.startTime)
            ? current.startTime
            : validStartTimes[0] ?? current.startTime;
          next.endTime = addMinutesToTime(next.startTime, 30);
        } else {
      ...
      if (key === "day" && next.isBreak) {
        const nextDay = value as TimetableDay;
        const validStartTimes =
          next.className === selectedClass && next.section === selectedSection
            ? getValidWeeklyBreakStartTimes()
            : getValidBreakStartTimes(nextDay);
        if (!validStartTimes.includes(next.startTime)) {
          next.startTime = validStartTimes[0] ?? next.startTime;
          next.endTime = addMinutesToTime(next.startTime, next.breakType === "Lunch Break" ? 60 : 30);
        }
      }
      return next;
    });
    ...
    setForm((current) => {
      const validStartTimes =
        current.className === selectedClass && current.section === selectedSection
          ? getValidWeeklyBreakStartTimes()
          : getValidBreakStartTimes(current.day);
      const resolvedStartTime = validStartTimes.includes(current.startTime)
        ? current.startTime
        : validStartTimes[0] ?? current.startTime;
      const nextEnd = addMinutesToTime(current.startTime, durationMinutes);
      const resolvedEnd = timeOptions.includes(addMinutesToTime(resolvedStartTime, durationMinutes))
        ? addMinutesToTime(resolvedStartTime, durationMinutes)
            ...
            <div className="grid gap-3 md:grid-cols-[1fr_1fr_1fr_auto]">
              ...
              <label className="block">
                ...
                <select
                  ...
                  className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-900"
                >
                  {availableSections.length === 0 ? <option value="">No sections available</option> : null}
              ...
              <div className="flex items-end">
                <div className="flex gap-3">
                  <Button className="min-h-10 rounded-xl px-3 py-2 text-sm" onClick={() => void loadGrid()} disabled={loadingMeta || !selectedClass || !selectedSection}>
                    {loadingGrid ? "Loading..." : "Load Timetable"}
                  </Button>
                  <Button
                    type="button"
                    variant="ghost"
                    className="min-h-10 rounded-xl bg-amber-50 px-3 py-2 text-sm text-amber-700 hover:bg-amber-100"
                    disabled={!canEdit || !selectedClass || !selectedSection}
                    onClick={() => {
                      const defaultBreakDay = selectedDay === "Sun" ? "Mon" : selectedDay;
                      const defaultBreakStart = getValidBreakStartTimes(defaultBreakDay)[0] ?? timetableSettings.schoolStartTime;
                      setForm({
                        className: selectedClass,
                        section: selectedSection,
                        subjectId: "",
                        ...
                        endTime: addMinutesToTime(defaultBreakStart, 30),
                        isBreak: true,
          ...
          <div className="grid gap-5 md:grid-cols-2">
            ...
            <label className="block">
              ...
              <select
                ...
                className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900"
                disabled={modal.mode === "view" || !(mode === "admin" && role === ROLES.ADMIN)}
              >
                {classOptions.map((item) => (
                  <option key={item.className} value={item.className}>
                    {item.className}
            ...
            <label className="block">
              ...
              <select
                ...
                disabled={modal.mode === "view"}
              >
                {(form.isBreak ? availableBreakStartTimeOptions : timeOptions.slice(0, -1)).map((time) => (
            ...
            {form.isBreak ? (
              <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-700 md:col-span-2">
                Break start times are limited to boundaries that do not cut through an existing class.
              </div>
            ) : null}
          </div>
