...
const getLeaveStatusLabel = (status: LeaveRecord["status"]) => {
  switch (status) {
    case "Pending_HR":
      return "Sent To HR";
    case "Pending_Admin":
      return "Sent To Admin & Principal";
    case "Rejected_By_HR":
      return "Rejected By HR";
    case "Rejected_By_Admin":
      return "Rejected By Admin / Principal";
    default:
      return "Approved";
  }
};
...
export const StaffDetailPage = () => {
  ...
  const staffListPath = isPrincipalArea ? "/principal/staff" : "/dashboard/staff";

  const salaryRows = useMemo(
    () => [...salary].sort((left, right) => right.month.localeCompare(left.month)),
    [salary],
  );
  ...
  const attendanceBySubject = useMemo(() => {
    const grouped = attendance.reduce((map, row) => {
      ...
      const current = map.get(key) ?? { label: key, sessions: 0 };
      current.sessions += 1;
      map.set(key, current);
      return map;
    }, new Map<string, { label: string; sessions: number }>());

    return Array.from(grouped.values()).sort((left, right) => right.sessions - left.sessions);
  }, [attendance]);

  const timetableByDay = useMemo(() => {
    const grouped = timetableSlots.reduce((map, row) => {
      map.set(row.day, [...(map.get(row.day) ?? []), row]);
      return map;
    }, new Map<string, TimetableSlotRecord[]>());

    return Array.from(grouped.entries()).map(([day, rows]) => ({
      day,
      slots: rows.length,
      classes: rows
        .map((row) => `${row.className}/${row.section}`)
    ...
    <div className="space-y-6">
      ...
      <Card className="border-slate-200 bg-white p-0 shadow-sm">
        ...
        <div className="grid gap-4 border-b border-slate-200 px-6 py-5 md:grid-cols-4">
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-slate-500">Base Salary</p>
            <p className="mt-2 text-lg font-semibold text-slate-900">
              {staff.monthlySalary != null ? staff.monthlySalary.toLocaleString() : "-"}
            </p>
          </div>
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-slate-500">Total Paid</p>
            <p className="mt-2 text-lg font-semibold text-slate-900">{summary.totalSalaryPaid}</p>
          </div>
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-slate-500">Salary Records</p>
            <p className="mt-2 text-lg font-semibold text-slate-900">{summary.salaryCount}</p>
          </div>
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-slate-500">Unpaid Records</p>
            <p className="mt-2 text-lg font-semibold text-slate-900">{summary.unpaidSalaryCount}</p>
          </div>
        </div>
              ...
              {salaryRows.length === 0 ? (
                ...
                salaryRows.map((row) => (
                  <tr key={row.id}>
                    ...
                    <td className="px-6 py-4">
                      {isPrincipalArea ? (
                        <Button type="button" variant="ghost" className="bg-slate-100 text-slate-700 hover:bg-slate-200" disabled>
                          Read Only
                        </Button>
                      ) : (
      ...
      <Card className="border-slate-200 bg-white p-0 shadow-sm">
        <div className="border-b border-slate-200 px-6 py-5">
          <h2 className="text-xl font-semibold text-slate-900">Teaching & Workload Summary</h2>
          <p className="mt-2 text-sm text-slate-500">
            Subject attendance coverage, timetable load, and class assignment summary.
          </p>
        </div>
        <div className="grid gap-6 p-6 xl:grid-cols-2">
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-slate-500">Attendance By Subject</p>
            <div className="mt-4 space-y-3">
              {attendanceBySubject.length === 0 ? (
                <p className="text-sm text-slate-500">No subject attendance sessions found.</p>
              ) : (
          ...
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-slate-500">Timetable By Day</p>
            <div className="mt-4 space-y-3">
              {timetableByDay.length === 0 ? (
                <p className="text-sm text-slate-500">No timetable slots assigned.</p>
              ) : (
                timetableByDay.map((row) => (
                  <div key={row.day} className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                    <div className="flex items-center justify-between gap-4">
                    ...
                    <p className="mt-2 text-xs text-slate-500">
                      {row.classes.length > 0 ? row.classes.join(", ") : "No class labels"}
                    </p>
                  </div>
                ))
              )}
