...
import { STAFF_WORKSPACES, normalizeStaffWorkspace } from "../../config/staffWorkspaces";
...
type StaffAttendancePageMode = "manage" | "self";
...
const StaffAttendancePage = ({ mode = "manage" }: { mode?: StaffAttendancePageMode }) => {
  ...
  const staffWorkspace = normalizeStaffWorkspace(currentStaff?.role);
  const canManageAttendanceByRole =
    role === ROLES.ADMIN || (role === ROLES.STAFF && staffWorkspace === STAFF_WORKSPACES.HR);
  const canManageAttendance = mode === "manage" && canManageAttendanceByRole;
  const effectiveStaffId = role === ROLES.STAFF && !canManageAttendance ? currentStaff?.id ?? "" : "";
  ...
  useEffect(() => {
    ...
    void (async () => {
      setLoading(true);
      try {
        const [employeeData, leaveData] = canManageAttendance
          ? await Promise.all([listEmployees(), listLeaves()])
          : [[], []];
        if (!active) return;
  ...
  const loadMonthlyAttendance = async (month: string, staffId = effectiveStaffId) => {
    const attendance = await listStaffAttendance({ month, staffId });
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const attendance = await listStaffAttendance({ month: selectedMonth, staffId: effectiveStaffId });
        if (!active) return;
        setRecords(attendance);
        setError("");
      } catch (loadError) {
  ...
  }, [effectiveStaffId, selectedMonth]);
  ...
  useEffect(() => {
    ...
    activeEmployees.forEach((employee) => {
      ...
      const status: StaffAttendanceStatus =
        existing?.status ??
        (isOnApprovedLeave ? "On Leave" : "Present");
  ...
  const metrics = useMemo(() => {
    ...
    const totalStaff = canManageAttendance ? activeEmployees.length : (currentStaff ? 1 : selectedDateRows.length);
    ...
    return {
      ...
      absentCount,
      leaveCount,
      coverage: totalStaff > 0 ? Math.round((markedCount / totalStaff) * 100) : 0,
    };
  }, [activeEmployees.length, canManageAttendance, currentStaff, records, selectedDate]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await loadMonthlyAttendance(selectedMonth, effectiveStaffId);
      setError("");
    } catch (saveError) {
  ...
  const canSaveAttendance = canManageAttendance && !saving && activeEmployees.length > 0;

  return (
    <div className="space-y-6">
      <AdminPageHeader
        title={canManageAttendance ? "Staff Attendance" : "My Attendance"}
        description={
          canManageAttendance
            ? "Mark HR-side attendance for today only, with month-wise history and role-wise filters."
            : "Review the attendance already marked by HR for your account."
        }
        action={
          canManageAttendance ? (
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
        <CompactMetricCard
          label={canManageAttendance ? "Active Staff" : "Records Loaded"}
          value={String(canManageAttendance ? metrics.totalStaff : records.length)}
          detail={canManageAttendance ? "Employees eligible for the selected date." : `Attendance records visible for ${formatMonthLabel(selectedMonth)}.`}
          icon={<MetricIcon />}
        />
      ...
      <Card className="border-slate-200 bg-white shadow-sm">
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
          <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
            <p className="text-sm font-medium text-slate-700">Attendance Date</p>
            <p className="mt-2 text-sm font-semibold text-slate-900">{selectedDate}</p>
            <p className="mt-1 text-xs text-slate-500">
              {canManageAttendance ? "Only today can be marked by HR." : "This page shows the attendance HR has already marked."}
            </p>
          </div>
          ...
          <label className="block">
            ...
            <input
              ...
              placeholder={canManageAttendance ? "Search staff..." : "Search attendance..."}
              className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 outline-none focus:border-brand-400 focus:ring-2 focus:ring-brand-200"
            />
          </label>
          <label className="block">
      ...
      <Card className="border-slate-200 bg-white p-0 shadow-sm">
        <div className="border-b border-slate-200 px-6 py-5">
          ...
          <p className="mt-2 text-sm text-slate-500">
            Mark attendance for {selectedDate}. Approved leave is prefilled as absent so HR can finish today&apos;s roster quickly.
          </p>
        </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                ...
                filteredEmployees.map((employee) => {
                  ...
                  const isLeaveLocked = isPrefilledLeave && (!draft?.status || draft.status === "On Leave");
                  ...
                  return (
                    <tr key={employee.id}>
                      ...
                      <td className="px-6 py-4 align-top">
                        {isLeaveLocked ? (
                          <span className={getStatusBadgeClassName("On Leave")}>On Leave</span>
                        ) : (
      ...
      <DataTable
        ...
        description={
          canManageAttendance
            ? `Saved records for ${formatMonthLabel(selectedMonth)}.`
            : `HR-marked attendance records for ${formatMonthLabel(selectedMonth)}.`
        }
        data={historyRows}
        getRowId={(row) => row.id}
...
export const MyAttendancePage = () => <StaffAttendancePage mode="self" />;
