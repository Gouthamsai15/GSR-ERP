...
const leaveStatusMeta: Record<LeaveRecord["status"], { label: string; className: string; stage: string }> = {
  ...
  Rejected_By_Admin: {
    label: "Rejected By Admin / Principal",
    className: "rounded-full border border-rose-200 bg-rose-50 px-3 py-1 text-xs font-semibold text-rose-700",
    stage: "Closed At Leadership Stage",
  },
};
...
const leaveTimelineSteps = [
  ...
  { key: "admin", title: "Leadership Review", description: "Admin or principal gives the final approval or rejection." },
...
export const AttendancePage = () => {
  ...
  const [filterDate, setFilterDate] = useState(getIndiaTodayIso());
  ...
  const [teacherSlots, setTeacherSlots] = useState<TimetableSlotRecord[]>([]);
  ...
  useEffect(() => {
    ...
    let active = true;
    void Promise.all([getStaffByUserId(user.id), loadMyTeachingTimetable(user.id)])
      .then(([staff, slots]) => {
        if (!active) return;
  ...
  }, [isStaffUser, queryClass, teacherSlots, user?.id]);
  ...
  }, [filterClass, isStaffUser, querySection, teacherSlots, user?.id]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        setSubjects([]);
        setFilterSubject("");
        setError(loadError instanceof Error ? loadError.message : "Failed to load attendance subjects.");
      }
    })();
  }, [filterClass, filterSection, filterDate, isStaffUser, teacherSlots, teacherStaffId]);

  useEffect(() => {
    if (!isStaffUser) return;
    if (!filterClass || !filterSection || !filterDate || !filterSubject) return;
    if (loadingSession || savingSession || teacherAttendanceLocked) return;
    if (
      session &&
      session.className === filterClass &&
      session.section === filterSection &&
      session.date === filterDate &&
      session.subjectId === filterSubject
    ) {
      return;
    }
  ...
  }, [filterClass, filterSection, filterDate, filterSubject, isStaffUser, savingSession, teacherAttendanceLocked]);
  ...
  return (
    <div className="space-y-6">
        ...
        action={
          isStaffUser ? (
            <Button
              ...
              className={
                checkingGeoStatus
                  ? "w-full bg-sky-600 text-white hover:bg-sky-700 disabled:opacity-100 disabled:text-white md:w-auto"
                  : teacherGeoChecked && teacherGeoAllowed
                    ? "w-full bg-emerald-600 text-white hover:bg-emerald-700 disabled:opacity-100 disabled:text-white md:w-auto"
                    : teacherGeoChecked && !teacherGeoAllowed
                      ? "w-full bg-rose-600 text-white hover:bg-rose-700 disabled:opacity-100 disabled:text-white md:w-auto"
                      : "w-full bg-amber-500 text-white hover:bg-amber-600 disabled:opacity-100 disabled:text-white md:w-auto"
              }
              onClick={() => {
                void verifyTeacherLocation().catch(() => undefined);
              }}
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-6">
            ...
            <div className="flex items-end sm:col-span-2 lg:col-span-1">
              <Button
                ...
                disabled={loadingSession || loadingGeoSettings || checkingGeoStatus || teacherAttendanceLocked}
                fullWidth
              >
      ...
      <Card className="border-slate-200 bg-white p-0 shadow-sm">
        {!session ? (
          <div className="flex min-h-[220px] items-center justify-center px-6 py-10 text-center text-slate-500">
              ...
              {session.rows.map((row) => (
                <div
                  ...
                  className={`rounded-3xl border px-4 py-4 shadow-sm ${
                    editedIds.has(row.studentId) ? "border-brand-200 bg-brand-50/50" : "border-slate-200 bg-white"
                  }`}
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <p className="truncate text-base font-semibold text-slate-900">{row.studentName}</p>
                      <p className="mt-1 text-xs font-medium uppercase tracking-[0.18em] text-slate-500">
                        {row.studentCode ?? row.studentId}
                      </p>
                    </div>
                    {row.attendanceId ? (
                      <Link to={`/dashboard/attendance/${row.attendanceId}`} className="shrink-0">
                ...
                <tbody className="divide-y divide-slate-100">
                  {session.rows.map((row) => (
                    <tr key={row.studentId} className={editedIds.has(row.studentId) ? "bg-brand-50/50" : "hover:bg-slate-50"}>
                      ...
                      <td className="px-6 py-4 text-slate-700">{row.studentCode ?? row.studentId}</td>
      ...
      {session ? (
        <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:justify-end">
          <Button
            type="button"
            variant="ghost"
            className="w-full bg-rose-50 text-rose-700 hover:bg-rose-100 sm:w-auto"
          ...
          <Button
            ...
            disabled={savingSession || checkingGeoStatus || loadingGeoSettings || teacherAttendanceLocked}
      ...
      <Card className="border-slate-200 bg-white p-0 shadow-sm">
        ...
        <div className="md:hidden space-y-3 px-4 py-4">
          {loadingSavedSessions ? (
            <div className="rounded-3xl border border-slate-200 bg-slate-50 px-4 py-6 text-center text-slate-500">
            ...
            savedSessions.map((item) => (
              <div key={`${item.className}-${item.section}-${item.subjectId}-${item.date}`} className="rounded-3xl border border-slate-200 bg-white px-4 py-4 shadow-sm">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-sm font-semibold text-slate-900">
                      {item.className} / {item.section}
                    </p>
                    <p className="mt-1 text-sm text-slate-600">{item.subjectName}</p>
                    <p className="mt-1 text-xs uppercase tracking-[0.18em] text-slate-500">{item.date}</p>
                  </div>
      ...
      {error ? (
        <Card className="border-amber-200 bg-amber-50 shadow-sm">
          ...
          <p className="mt-2 text-xs text-amber-700">
            Exam sections and schedules are loaded from the current school workspace data.
          </p>
        </Card>
      ) : null}
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {session.rows.map((row) => (
                  <tr key={row.studentId} className={editedIds.has(row.studentId) ? "bg-brand-50/50" : "hover:bg-slate-50"}>
                    <td className="px-6 py-4 font-medium text-slate-700">{row.studentCode ?? row.studentId}</td>
                    <td className="px-6 py-4 font-medium text-slate-900">{row.studentName}</td>
...
export const ResultsPage = () => {
  ...
  const [selectedExamSubjects, setSelectedExamSubjects] = useState<ExamSubjectOption[]>([]);
  ...
  useEffect(() => {
    ...
    void Promise.all([listResults(selectedExamId), listExamSubjects(selectedExamId)])
      .then(([resultRows, subjectRows]) => {
        if (!active) return;
        setSelectedExamResults(resultRows);
        setSelectedExamSubjects(subjectRows);
        const availableClasses = Array.from(new Set(resultRows.map((item) => item.className).filter((value): value is string => Boolean(value)))).sort((left, right) => left.localeCompare(right));
        const nextClass = queryClass && availableClasses.includes(queryClass) ? queryClass : "";
        const availableSections = Array.from(
          new Set(
            resultRows
              .filter((item) => !nextClass || item.className === nextClass)
  ...
  const classNames = useMemo(
    () => Array.from(new Set(selectedExamResults.map((item) => item.className).filter((value): value is string => Boolean(value)))).sort((left, right) => left.localeCompare(right)),
    [selectedExamResults],
  );
  ...
  const classRows = useMemo(
    ...
    [classNames, selectedExamResults],
  );
  ...
  const sectionRows = useMemo(
    () =>
      sectionNames.map((section) => ({
        id: section,
        section,
        studentCount: selectedExamResults.filter((item) => item.className === selectedClassName && item.section === section).length,
      })),
    [selectedClassName, selectedExamResults, sectionNames],
  );

  const activeSectionRows = useMemo(
    () =>
      selectedExamResults
        .filter((item) => (!selectedClassName || item.className === selectedClassName) && (!selectedSection || item.section === selectedSection))
  ...
  const handleDelete = async (item: ResultRecord) => {
    ...
    try {
      ...
      if (selectedExamId) {
        setLoadingResults(true);
        const [resultRows, subjectRows] = await Promise.all([listResults(selectedExamId), listExamSubjects(selectedExamId)]);
        setSelectedExamResults(resultRows);
        setSelectedExamSubjects(subjectRows);
      }
    } catch (deleteError) {
      ...
      if (selectedExamId) setLoadingResults(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <AdminPageHeader
        title="Results"
        description="Pick an exam first, then choose a class and section to review results and paper schedules."
        action={<Link to="/dashboard/exams"><Button>Go To Exams</Button></Link>}
      />

      <DataTable
        title="Exam List"
        description="Open an exam to see the class, section, and student result breakdown."
        data={exams}
        columns={[
          {
            key: "name",
            ...
            render: (exam) => (
              <div>
                ...
                <p className="text-xs text-slate-500">{exam.subjectCount} subjects</p>
              </div>
            ),
            emphasis: true,
          },
          { key: "className", label: "Class", render: (exam) => exam.className },
          { key: "section", label: "Section", render: (exam) => exam.section },
          { key: "status", label: "Status", render: (exam) => exam.status },
          { key: "date", label: "Dates", render: (exam) => `${exam.startDate} to ${exam.endDate}` },
        ]}
        ...
        emptyMessage="No exams found."
        renderActions={(exam) => (
          <Button type="button" onClick={() => openExam(exam.id)}>
            Open
          </Button>
        )}
      />
      ...
      {selectedExamId ? (
        <div className="space-y-5">
          <Card className="border-slate-200 bg-white shadow-sm">
            <div className="flex flex-col gap-4 border-b border-slate-200 pb-4 md:flex-row md:items-start md:justify-between">
              <div>
                <p className="text-xs uppercase tracking-[0.2em] text-brand-600">Selected Exam</p>
                <h2 className="mt-2 text-xl font-semibold text-slate-900">{selectedExam?.name ?? "Exam"}</h2>
                <p className="mt-1 text-sm text-slate-500">
                  {selectedExam?.startDate ?? "-"} to {selectedExam?.endDate ?? "-"} • {selectedExam?.status ?? "-"} • {selectedExamResults.length} result records
                </p>
              </div>
              <div className="grid gap-3 text-sm md:min-w-[260px]">
                <div className="rounded-2xl bg-slate-50 px-4 py-3">
                  ...
                  <p className="mt-1 font-semibold text-slate-900">{selectedExamSubjects.length}</p>
                </div>
                <div className="rounded-2xl bg-slate-50 px-4 py-3">
                  <p className="text-xs uppercase tracking-[0.18em] text-slate-500">Classes</p>
                  <p className="mt-1 font-semibold text-slate-900">{classNames.length}</p>
                </div>
              </div>
            </div>
            ...
            <div className="mt-5 grid gap-3 md:grid-cols-2">
              {selectedExamSubjects.length === 0 ? (
                <p className="text-sm text-slate-500">No paper schedule found for this exam.</p>
              ) : (
          ...
          <DataTable
            title="Classes in Exam"
            description="Choose a class to see the sections that have marks saved."
            data={classRows}
            columns={[
              { key: "className", label: "Class", render: (row) => row.className, emphasis: true },
              { key: "sectionCount", label: "Sections", render: (row) => row.sectionCount },
            ]}
            getRowId={(row) => row.id}
            emptyMessage="No classes found for this exam."
            renderActions={(row) => (
              <Button
                type="button"
                onClick={() => {
                  setSelectedClassName(row.className);
                  setSelectedSection("");
                }}
              >
                Select
          ...
          {selectedClassName ? (
            <DataTable
              title={`Sections in ${selectedClassName}`}
              description="Pick the section to open the student results."
              data={sectionRows}
              columns={[
                { key: "section", label: "Section", render: (row) => row.section, emphasis: true },
                { key: "studentCount", label: "Students", render: (row) => row.studentCount },
              ]}
              getRowId={(row) => row.id}
              emptyMessage="No sections found for this class."
              renderActions={(row) => (
                <Button type="button" onClick={() => setSelectedSection(row.section)}>
                  Open
                </Button>
              )}
            />
          ...
          <Card className="border-slate-200 bg-white p-0 shadow-sm">
            {loadingResults ? (
              <div className="flex min-h-[220px] items-center justify-center px-6 text-slate-500">Loading results...</div>
            ) : !selectedClassName ? (
              <div className="flex min-h-[220px] items-center justify-center px-6 text-center text-slate-500">
              ...
              <div className="flex min-h-[220px] items-center justify-center px-6 text-center text-slate-500">
                Select a section to view the student results.
              </div>
            ) : activeSectionRows.length === 0 ? (
              <div className="flex min-h-[220px] items-center justify-center px-6 text-center text-slate-500">
                No results found for this class and section.
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="min-w-full text-left text-sm">
        ...
        <Card className="border-slate-200 bg-white shadow-sm">
          <p className="text-sm text-slate-500">Open an exam from the list above to view classes, sections, and results.</p>
        </Card>
      )}
    </div>
  );
...
export const LeavesPage = () => {
  ...
  return (
    <CrudPage<LeaveRecord>
      ...
      description={
        isTeacherWorkspace
          ? "Request leave first. HR reviews it, then admin or principal can finalize the decision."
          : isHrWorkspace
            ? "Review only HR-stage leave requests, then forward them to admin and principal or reject them."
            : "Review leadership-stage leave requests and make the final approval decision."
      }
      ...
      filters={[
        {
          ...
          options: [
            ...
            { label: "Pending Admin / Principal", value: "Pending_Admin" },
            { label: "Approved", value: "Approved" },
            { label: "Rejected By HR", value: "Rejected_By_HR" },
            { label: "Rejected By Admin / Principal", value: "Rejected_By_Admin" },
          ],
          getValue: (item) => item.status,
        },
      ]}
        ...
        isHrWorkspace && item.status === "Pending_HR" ? (
          ...
          <>
            <Input
              ...
              placeholder="Leadership comment"
              className="max-w-[220px]"
            />
            <Button
              variant="ghost"
    ...
    fields={[
      {
        ...
        required: true,
        placeholder: "AP 15 GS 2005",
        maxLength: 13,
        inputMode: "text",
        transform: formatVehicleNumberInput,
      },
      {
        key: "driverName",
        label: "Driver Name",
...
export const ApplicantsPage = () => {
  ...
  const openStudentForm = (item: ApplicantRecord) => {
    navigate("/dashboard/students", {
      state: {
        applicantDraft: {
          ...
          fatherMobileNumber: item.parentPhone ?? "",
        },
      },
    });
  };
  ...
  return (
    <>
      <CrudPage<ApplicantRecord>
        key={refreshKey}
        title="Applicants"
        description="Track applicants, maintain records, and move approved admissions into the student entry workflow."
      ...
      >
        <form
          ...
          onSubmit={async (event) => {
            ...
            setApprovalError("");
            try {
              if (!window.confirm(`Approve ${approvalTarget.name} and move them to the student details form?`)) {
                setApproving(false);
                return;
              }
              await approveApplicant({ applicantId: approvalTarget.id });
              closeApprove();
        ...
        >
          <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600">
            Approval will open the full student details form with the applicant name and class already filled in.
          </div>
      ...
      <DetailSection title="Students List">
        {record.rows.length === 0 ? (
          ...
          record.rows.map((row) => (
            <DetailField
              key={row.studentId}
              label={row.studentCode ? `${row.studentName} (${row.studentCode})` : row.studentName}
              value={
...
export const ResultDetailPage = () => {
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      doc.setFontSize(18);
      doc.text("GSR ERP Result Summary", 14, y);
      y += 12;

      doc.setFontSize(11);
      doc.text(`Student: ${record.studentName}`, 14, y);
      doc.text(`Student ID: ${record.studentCode ?? record.studentId}`, 110, y);
      y += 7;
      doc.text(`Class: ${record.className ?? "-"}`, 14, y);
      doc.text(`Section: ${record.section ?? "-"}`, 110, y);
      y += 7;
      doc.text(`Exam: ${record.examName}`, 14, y);
      y += 12;
      ...
      doc.save(`${record.studentName}-${record.examName}-result.pdf`);
    } finally {
        ...
        sections={[
          {
            ...
            fields: [
              ...
              { label: "Student ID", value: record.studentCode ?? record.studentId },
              { label: "Class", value: record.className ?? "-" },
              { label: "Section", value: record.section ?? "-" },
            ],
          },
