...
const createBootstrapSession = (): AppSession => ({
  accessToken: BOOTSTRAP_ACCESS_TOKEN,
  refreshToken: null,
  expiresAt: null,
  uid: DEFAULT_ADMIN.user.id,
...
const preloadAuthData = (resolved: ResolvedAuthState) => {
  if (!resolved.user || !resolved.role || typeof window === "undefined") return;

  window.setTimeout(() => {
    void import("./adminService")
      .then(({ preloadCoreDashboardData }) => preloadCoreDashboardData(resolved.role))
      .catch(() => undefined);
  }, 0);
};
...
const extractSessionHints = (
  ...
  const hintedName = String(claims.name ?? user.displayName ?? user.email?.split("@")[0] ?? "GSR ERP User").trim() || "GSR ERP User";
...
const resolveAuthFromUser = async (firebaseUser: FirebaseUser) => {
  ...
  try {
    ...
    const user = mapUser({
      ...
      name: backendContext.user.name ?? firebaseUser.displayName ?? firebaseUser.email?.split("@")[0] ?? "GSR ERP User",
...
export const loginStaff = async (email: string, password: string) => {
  ...
  if (isDefaultAdminLogin(trimmedEmail, password)) {
    return { user: DEFAULT_ADMIN.user, role: ROLES.SUPER_ADMIN, school: null, session: createBootstrapSession() };
  }
...
export const restoreSession = async () => {
  const bootstrapAdmin = getBootstrapAdmin();
  if (bootstrapAdmin) {
    return { user: bootstrapAdmin, role: bootstrapAdmin.role, school: null, session: createBootstrapSession() };
  }
