...
const normalizeComparableText = (value: string | null | undefined) => String(value ?? "").trim().toLowerCase();

const formatVehicleNumber = (value: string | null | undefined) => {
  const input = String(value ?? "").toUpperCase();
  const accepted: string[] = [];
  const slots: Array<"L" | "D"> = ["L", "L", "D", "D", "L", "L", "D", "D", "D", "D"];

  for (const char of input) {
    if (accepted.length >= slots.length) break;
    const slot = slots[accepted.length];
    if (slot === "L" && /[A-Z]/.test(char)) {
      accepted.push(char);
    } else if (slot === "D" && /\d/.test(char)) {
      accepted.push(char);
    }
  }
  ...
  const parts = [
    ...
    accepted.slice(2, 4).join(""),
    accepted.slice(4, 6).join(""),
    accepted.slice(6, 10).join(""),
  ].filter(Boolean);
  return parts.join(" ");
};

const isValidVehicleNumber = (value: string | null | undefined) => /^[A-Z]{2} \d{2} [A-Z]{2} \d{4}$/.test(String(value ?? "").trim());

const formatDriverName = (value: string | null | undefined) =>
  String(value ?? "")
    .replace(/[^a-zA-Z\s]/g, "")
    .replace(/\s+/g, " ")
    .trim();
...
const mapFirebaseUserRow = (id: string, data: Record<string, unknown>): UsersRow => ({
  id,
  name: getFirebaseString(data, ["name"]) ?? "GSR ERP User",
...
const mapFirebaseLeaveRow = (id: string, data: Record<string, unknown>): LeaveRow => ({
  id,
  employee_id: getFirebaseString(data, ["employeeId", "employee_id"]),
  staff_id: getFirebaseString(data, ["staffId", "staff_id"]),
  start_date: getFirebaseString(data, ["startDate", "start_date"]) ?? "",
  end_date: getFirebaseString(data, ["endDate", "end_date"]) ?? "",
  status: getFirebaseString(data, ["status"]) ?? "Pending_HR",
  reason: getFirebaseString(data, ["reason"]),
  hr_comment: getFirebaseString(data, ["hrComment", "hr_comment"]),
  admin_comment: getFirebaseString(data, ["adminComment", "admin_comment"]),
});
...
const mapFirebaseApplicantRow = (id: string, data: Record<string, unknown>): ApplicantRow => ({
  id,
  name: getFirebaseString(data, ["name"]),
  email: getFirebaseString(data, ["email"]),
  class: getFirebaseString(data, ["class", "className"]),
  student_name: getFirebaseString(data, ["studentName", "student_name", "name"]),
  class_applied: getFirebaseString(data, ["classApplied", "class_applied", "className", "class"]),
  status: getFirebaseString(data, ["status"]) ?? "Pending",
  parent_name: getFirebaseString(data, ["parentName", "parent_name"]),
  parent_email: getFirebaseString(data, ["parentEmail", "parent_email"]),
  parent_phone: getFirebaseString(data, ["parentPhone", "parent_phone"]),
  created_at: getFirebaseString(data, ["createdAt", "created_at"]),
});

const mapFirebaseTimetableRow = (id: string, data: Record<string, unknown>): TimetableRow => ({
  id,
  class: getFirebaseString(data, ["className", "class"]) ?? "",
  section: getFirebaseString(data, ["section"]) ?? "",
  subject_id: getFirebaseString(data, ["subjectId", "subject_id"]),
  teacher_id: getFirebaseString(data, ["teacherId", "teacher_id"]),
  day: getFirebaseString(data, ["day"]) ?? "Mon",
  start_time: getFirebaseString(data, ["startTime", "start_time"]) ?? "08:00",
  end_time: getFirebaseString(data, ["endTime", "end_time"]) ?? "09:00",
  is_break: getFirebaseBoolean(data, ["isBreak", "is_break"]),
  break_type: (getFirebaseString(data, ["breakType", "break_type"]) as TimetableRow["break_type"]) ?? null,
  break_label: getFirebaseString(data, ["breakLabel", "break_label"]),
  is_cancelled: getFirebaseBoolean(data, ["isCancelled", "is_cancelled"]),
  cancellation_reason: getFirebaseString(data, ["cancellationReason", "cancellation_reason"]),
});

const mapFirebaseAttendanceRow = (id: string, data: Record<string, unknown>): AttendanceRow => ({
  id,
  student_id: getFirebaseString(data, ["studentId", "student_id"]) ?? "",
  subject_id: getFirebaseString(data, ["subjectId", "subject_id"]),
  date: getFirebaseString(data, ["date"]),
  status: getFirebaseString(data, ["status"]),
  teacher_id: getFirebaseString(data, ["teacherId", "teacher_id"]),
  created_at: getFirebaseString(data, ["createdAt", "created_at"]),
});
...
const getFirestoreStudentsByClassSection = async (schoolId: string, className: string, section: string) => {
  const normalizedClassName = normalizeComparableText(className);
  const normalizedSection = normalizeComparableText(section);
  const docs = await getFirestoreDocsByFilterVariants(
    "students",
    [
      [
  ...
  const matchedStudents = docs
    .map((item) => mapFirebaseStudentRow(item.id, item.data))
    .filter(
      (student) =>
        normalizeComparableText(student.class) === normalizedClassName &&
        normalizeComparableText(student.section) === normalizedSection,
    );
  ...
  const schoolScopedStudents = await getFirestoreSchoolScopedDocs("students", schoolId);
  return schoolScopedStudents
    .map((item) => mapFirebaseStudentRow(item.id, item.data))
    .filter((student) => {
      const studentClass = normalizeComparableText(student.class);
      const studentSection = normalizeComparableText(student.section);
      return studentClass === normalizedClassName && studentSection === normalizedSection;
    });
};
...
const getDataCacheKey = (namespace: string) => `${getCurrentSchoolId() ?? "no-school"}::${namespace}`;
...
const DATA_STORE_CACHE_TTL_MS = 5 * 60 * 1000;
...
const loadTimetableClassOptionsCache = new Map<string, { expiresAt: number; value?: TimetableClassOption[]; promise?: Promise<TimetableClassOption[]> }>();
const backgroundRefreshPromises = new Map<string, Promise<unknown>>();

const getSchoolCacheKey = (namespace: string, schoolId: string) => `${schoolId}::${namespace}`;

const isPersistedCacheFresh = (loadedAt: number | undefined) =>
  typeof loadedAt === "number" && Date.now() - loadedAt < DATA_STORE_CACHE_TTL_MS;

const scheduleBackgroundRefresh = (key: string, loader: () => Promise<unknown>) => {
  const existing = backgroundRefreshPromises.get(key);
  if (existing) {
    return existing;
  }
  ...
  const promise = loader()
    .catch(() => undefined)
    .finally(() => {
      backgroundRefreshPromises.delete(key);
    });

  backgroundRefreshPromises.set(key, promise);
  return promise;
};
...
const invalidateSchoolScopedCache = (namespaces?: string[]) => {
  ...
  dataStore.getState().clearSchoolCache(schoolId);
...
const queueAuditEvent = (
  action: "CREATE" | "UPDATE" | "DELETE",
  module: string,
  recordId?: string | null,
  actorUserId?: string | null,
) => {
  void logAuditEvent(action, module, recordId, actorUserId).catch(() => {
    // Audit logging should never slow down or block successful form saves.
  });
};
...
const buildApplicantEmail = (values: Pick<ApplicantFormValues, "name" | "className" | "parentEmail" | "parentPhone" | "email">) => {
  const providedEmail = String(values.email ?? "").trim();
  if (providedEmail) {
  ...
  const namePart = clean(values.name);
  const classPart = clean(values.className);
  const parentHint = clean(values.parentEmail || values.parentPhone);
  const localPart = [namePart, classPart, parentHint]
    .filter((part) => Boolean(part))
    .join(".")
    .replace(/\.+/g, ".")
    .replace(/^\.+|\.+$/g, "");

  return `${localPart || "applicant"}@school.local`;
};
...
const invokeAdminAction = async <T>(action: string, payload: Record<string, unknown>) => {
  ...
  if (!response.ok || result.error) {
    ...
    if (response.status >= 500) {
      throw normalizeDuplicateActionError(
        new Error(
          result.error?.trim()
    ...
    throw normalizeDuplicateActionError(
      new Error(result.error ?? `Admin action failed (${response.status} ${response.statusText}).`),
    );
  }
...
const mapStudentRecord = (
  ...
  photoUrl: studentUser?.photo_url ?? item.photo_url ?? null,
...
const formatDuplicateDateLabel = (date: string | null | undefined) => {
  const normalizedDate = String(date ?? "").trim();
  return normalizedDate ? formatShortDateFromDateString(normalizedDate) : "the selected date";
};

const ensureUniqueSubjectName = async (name: string, currentSubjectId?: string | null) => {
  const normalizedName = normalizeLookupValue(name);
  if (!normalizedName) return;

  const duplicate = (await listSubjects()).find(
    (item) => item.id !== currentSubjectId && normalizeLookupValue(item.name) === normalizedName,
  );

  if (duplicate) {
    throw new Error(`Subject "${name.trim()}" already exists.`);
  }
};

const ensureUniqueHolidayEntry = async (
  holidayDate: string,
  title: string,
  ...
  const duplicateOnSameDate = holidays.find(
    (item) => item.id !== currentHolidayId && item.holidayDate === holidayDate,
  );

  if (duplicateOnSameDate) {
    throw new Error(`A holiday already exists on ${formatDuplicateDateLabel(holidayDate)}.`);
  }

  const duplicateTitle = holidays.find(
    (item) =>
      item.id !== currentHolidayId &&
      normalizeLookupValue(item.title) === normalizedTitle,
  );

  if (duplicateTitle) {
    throw new Error(`Holiday "${title.trim()}" already exists.`);
  }
};

const ensureUniqueExamEntries = async (
  values: Pick<ExamFormValues, "name" | "className" | "sections" | "section">,
  startDate: string,
  currentExamId?: string | null,
) => {
  const normalizedName = normalizeLookupValue(values.name);
  const normalizedClassName = normalizeLookupValue(values.className);
  ...
  for (const section of selectedSections) {
    ...
    const duplicate = exams.find(
      (item) =>
        ...
        normalizeLookupValue(item.className) === normalizedClassName &&
        normalizeLookupValue(item.section) === normalizedSection &&
        String(item.startDate ?? "") === startDate,
    );

    if (duplicate) {
      throw new Error(
        `Exam "${values.name.trim()}" already exists for ${values.className.trim()} ${section} on ${formatDuplicateDateLabel(startDate)}.`,
      );
    }
  }
};

const ensureUniqueFeeEntry = async (
  studentId: string,
  dueDate: string,
  currentFeeId?: string | null,
) => {
  const duplicate = (await listFees()).find(
    (item) => item.id !== currentFeeId && item.studentId === studentId && String(item.dueDate ?? "") === dueDate,
  );

  if (duplicate) {
    throw new Error(`A fee record already exists for this student on ${formatDuplicateDateLabel(dueDate)}.`);
  }
};

const ensureUniqueSalaryEntry = async (
  staffId: string,
  month: string,
  currentSalaryId?: string | null,
) => {
  const duplicate = (await listSalary()).find(
    (item) => item.id !== currentSalaryId && item.staffId === staffId && item.month === month,
  );

  if (duplicate) {
    throw new Error(`Salary for this employee already exists for ${month}.`);
  }
};

const ensureUniqueVehicleEntry = async (
  vehicleName: string,
  driverPhone: string,
  currentVehicleId?: string | null,
) => {
  const normalizedVehicleName = normalizeLookupValue(vehicleName);
  const normalizedDriverPhone = normalizeLookupValue(driverPhone);
  const duplicate = (await listVehicles()).find(
    (item) =>
      item.id !== currentVehicleId &&
      (normalizeLookupValue(item.vehicleName) === normalizedVehicleName ||
        (normalizedDriverPhone && normalizeLookupValue(item.driverPhone) === normalizedDriverPhone)),
  );
  ...
  if (normalizedDriverPhone && normalizeLookupValue(duplicate.driverPhone) === normalizedDriverPhone) {
    throw new Error(`Driver phone number "${driverPhone.trim()}" is already assigned to another vehicle.`);
  }

  throw new Error(`Vehicle "${vehicleName.trim()}" already exists.`);
};

const ensureUniqueRouteEntry = async (routeName: string, currentRouteId?: string | null) => {
  const normalizedRouteName = normalizeLookupValue(routeName);
  const duplicate = (await listTransportRoutes()).find(
    (item) => item.id !== currentRouteId && normalizeLookupValue(item.routeName) === normalizedRouteName,
  );

  if (duplicate) {
    throw new Error(`Route "${routeName.trim()}" already exists.`);
  }
};

const ensureUniqueApplicantEntry = async (
  values: Pick<ApplicantFormValues, "name" | "className" | "parentEmail" | "parentPhone" | "email">,
  currentApplicantId?: string | null,
) => {
  const normalizedEmail = normalizeLookupValue(values.email ?? "");
  const normalizedParentEmail = normalizeLookupValue(values.parentEmail);
  const normalizedParentPhone = normalizeLookupValue(values.parentPhone);
  const normalizedName = normalizeLookupValue(values.name);
  const normalizedClassName = normalizeLookupValue(values.className);
  const duplicate = (await listApplicants()).find((item) => {
    if (item.id === currentApplicantId) {
      return false;
    }

    const applicantEmail = normalizeLookupValue(item.email);
    const applicantParentEmail = normalizeLookupValue(item.parentEmail);
    const applicantParentPhone = normalizeLookupValue(item.parentPhone);
    const matchesPrimaryIdentity =
      (normalizedEmail && applicantEmail === normalizedEmail) ||
      (normalizedParentEmail && applicantParentEmail === normalizedParentEmail) ||
      (normalizedParentPhone && applicantParentPhone === normalizedParentPhone);

    const matchesStudentIdentity =
      normalizeLookupValue(item.name) === normalizedName &&
      normalizeLookupValue(item.className) === normalizedClassName;

    return matchesPrimaryIdentity || matchesStudentIdentity;
  });

  if (duplicate) {
    throw new Error(`Applicant "${values.name.trim()}" already exists.`);
  }
};

const normalizeDuplicateActionError = (error: unknown) => {
  ...
  if (lowerMessage.includes("duplicate key value") || lowerMessage.includes("unique constraint")) {
    if (lowerMessage.includes("email")) return new Error("This email already exists.");
    if (lowerMessage.includes("phone") || lowerMessage.includes("mobile")) return new Error("This mobile number already exists.");
    if (lowerMessage.includes("aadhar")) return new Error("This Aadhar number already exists.");
    if (lowerMessage.includes("student_code")) return new Error("This student code already exists.");
...
export const createClass = async (values: ClassFormValues) =>
  (async () => {
    ...
    await Promise.all([
      ensureUniqueClassSection(payload.className, payload.section),
      ensureUniqueRoomNumber(payload.roomNumber),
    ]);
    ...
    queueAuditEvent("CREATE", "CLASS", classRecord.id);
    return classRecord;
  })();
...
export const updateClass = async (classId: string, values: ClassFormValues) =>
  (async () => {
    ...
    const payload = validateClassForm(values);
    await Promise.all([
      ensureUniqueClassSection(payload.className, payload.section, classId),
      ensureUniqueRoomNumber(payload.roomNumber, classId),
    ]);
    const classRecord = await invokeAdminAction<ClassRecord>("update_class", { id: classId, ...values });

    const classChanged =
      current.classRecord.className !== payload.className ||
      current.classRecord.section !== payload.section;

    if (classChanged && !firebaseDb) {
      const client = requireDatabaseClient();
    ...
    invalidateSchoolScopedCacheByPrefix(["timetable:", "attendance:", "student-by-user:"]);
    queueAuditEvent("UPDATE", "CLASS", classRecord.id);
    return classRecord;
  })();
...
export const createHoliday = async (values: HolidayFormValues) =>
  (async () => {
    ...
    const schoolId = requireCurrentSchoolId();
    await ensureUniqueHolidayEntry(payload.holidayDate, payload.title);
...
export const updateHoliday = async (holidayId: string, values: HolidayFormValues) =>
  (async () => {
    const payload = validateHolidayForm(values);
    const schoolId = requireCurrentSchoolId();
    await ensureUniqueHolidayEntry(payload.holidayDate, payload.title, holidayId);
...
export const createSubject = async (values: SubjectFormValues) =>
  (async () => {
    ...
    await ensureUniqueSubjectName(name);
...
export const updateSubject = async (subjectId: string, values: SubjectFormValues) =>
  (async () => {
    ...
    const schoolId = requireCurrentSchoolId();
    if (!name) throw new Error("Subject name is required.");
    await ensureUniqueSubjectName(name, subjectId);
...
export const listStaff = async (options?: { forceRefresh?: boolean }): Promise<StaffRecord[]> => {
  const cacheKey = getDataCacheKey("staff:all");
  const cached = dataStore.getState().getCachedPage<StaffRecord>("staffCache", cacheKey);
  if (!options?.forceRefresh && cached?.items?.length) {
    if (!isPersistedCacheFresh(cached.loadedAt)) {
      void scheduleBackgroundRefresh(cacheKey, () => listStaff({ forceRefresh: true }));
    }
    return cached.items;
  }
  ...
  return readSchoolScopedCache("staff", async () => {
    if (firebaseDb) {
      ...
      const records = staffRows.map((item) => ({
        id: item.id,
        userId: item.user_id,
        name: item.name ?? usersMap.get(item.user_id)?.name ?? "Unnamed staff",
        email: usersMap.get(item.user_id)?.email ?? "",
      ...
      dataStore.getState().setPageCache("staffCache", cacheKey, { items: records, nextCursor: null, loadedAt: Date.now() });
      return records;
    }
...
export const listStudents = async (options?: { forceRefresh?: boolean }): Promise<StudentRecord[]> => {
  const cacheKey = getDataCacheKey("students:all");
  const cached = dataStore.getState().getCachedPage<StudentRecord>("studentsCache", cacheKey);
  if (!options?.forceRefresh && cached?.items?.length) {
    if (!isPersistedCacheFresh(cached.loadedAt)) {
      void scheduleBackgroundRefresh(cacheKey, () => listStudents({ forceRefresh: true }));
    }
    return cached.items;
  }
  ...
  return readSchoolScopedCache("students", async () => {
    if (firebaseDb) {
      ...
      const records = studentRows.map((item) =>
        mapStudentRecord(
          item,
      ...
      dataStore.getState().setPageCache("studentsCache", cacheKey, { items: records, nextCursor: null, loadedAt: Date.now() });
      return records;
    }
...
export const getStudentByUserId = async (userId: string): Promise<StudentRecord | null> => {
...
export const getStaffByUserId = async (userId: string) => {
  return readSchoolScopedCache(`staff-by-user:${userId}`, async () => {
    if (firebaseDb) {
      const schoolId = requireCurrentSchoolId();
      let snapshot = await getFirstFirestoreDocByFilterVariants(
        "staff",
        [
          [
            { field: "schoolId", value: schoolId },
      ...
      if (!snapshot) {
        const targetUser = authStore.getState().user;
        const targetEmail =
          targetUser?.id === userId ? String(targetUser.email ?? "").trim().toLowerCase() : "";
        const targetName =
          targetUser?.id === userId ? String(targetUser.name ?? "").trim().toLowerCase() : "";

        if (targetEmail || targetName) {
          const staffDocs = await queryFirestoreCollectionCached({
            collectionName: "staff",
            filters: [],
          });
          const schoolScopedStaffDocs = staffDocs.filter((doc) => {
            const docSchoolId = getFirebaseString(doc.data, ["schoolId", "school_id"]) ?? "";
            return !docSchoolId || docSchoolId === schoolId;
          });

          const directMatch = schoolScopedStaffDocs.find((doc) => {
            const data = doc.data;
            const docUserId = getFirebaseString(data, ["userId", "user_id"]) ?? "";
            const docEmail = String(getFirebaseString(data, ["email"]) ?? "").trim().toLowerCase();
            return (
          ...
          if (directMatch) {
            snapshot = directMatch;
          }

          const candidateDocs = snapshot ? [snapshot] : schoolScopedStaffDocs;
          const candidateUserIds = Array.from(
            new Set(
              candidateDocs
                .map((doc) => getFirebaseString(doc.data, ["userId", "user_id"]))
                .filter((value): value is string => Boolean(value?.trim())),
            ),
          );
          const usersMap = await getUsersMap(candidateUserIds);

          const fallbackSnapshot = candidateDocs.find((doc) => {
            const data = doc.data;
            const linkedUserId = getFirebaseString(data, ["userId", "user_id"]) ?? "";
            const linkedUser = linkedUserId ? usersMap.get(linkedUserId) : undefined;
            const linkedEmail = String(linkedUser?.email ?? "").trim().toLowerCase();
            const docName = String(getFirebaseString(data, ["name"]) ?? "").trim().toLowerCase();
            return (
              (targetEmail && linkedEmail === targetEmail) ||
...
const mapFirebaseExamRow = (id: string, data: Record<string, unknown>): ExamRow => ({
  id,
  name: String(data.name ?? ""),
  class: String(data.class ?? data.className ?? ""),
  section: data.section == null ? "" : String(data.section),
  date: data.date == null ? null : String(data.date),
  exam_date: data.exam_date == null ? (data.examDate == null ? null : String(data.examDate)) : String(data.exam_date),
  start_date: data.start_date == null ? (data.startDate == null ? null : String(data.startDate)) : String(data.start_date),
  end_date: data.end_date == null ? (data.endDate == null ? null : String(data.endDate)) : String(data.end_date),
  exam_session:
    (data.exam_session as ExamRow["exam_session"]) ??
    (data.examSession as ExamRow["exam_session"]) ??
    "Full Day",
  status: (data.status as ExamRow["status"]) ?? "Draft",
  created_at: data.created_at == null ? (data.createdAt == null ? null : String(data.createdAt)) : String(data.created_at),
});

const mapFirebaseExamSubjectRow = (id: string, data: Record<string, unknown>): ExamSubjectRow => ({
  id,
  exam_id: String(data.exam_id ?? data.examId ?? ""),
  subject_id: String(data.subject_id ?? data.subjectId ?? ""),
  max_marks:
    data.max_marks == null
      ? (data.maxMarks == null ? null : Number(data.maxMarks))
      : Number(data.max_marks),
  exam_date: data.exam_date == null ? (data.examDate == null ? null : String(data.examDate)) : String(data.exam_date),
  exam_session:
    (data.exam_session as ExamSubjectRow["exam_session"]) ??
    (data.examSession as ExamSubjectRow["exam_session"]) ??
    null,
  start_time: data.start_time == null ? (data.startTime == null ? null : String(data.startTime)) : String(data.start_time),
  end_time: data.end_time == null ? (data.endTime == null ? null : String(data.endTime)) : String(data.end_time),
});
...
export const isExamMarksEntryOpen = (exam: Pick<ExamRecord, "subjectCount">) => exam.subjectCount > 0;

export const getExamMarksEntryAvailabilityMessage = (
  exam: Pick<ExamRecord, "subjectCount">,
) => {
  if (isExamMarksEntryOpen(exam)) {
    return "";
  }

  return "Marks entry opens after the exam paper schedule is created.";
};
...
const listAttendanceRows = async (filters?: {
  ...
  if (firebaseDb) {
    try {
      const schoolId = requireCurrentSchoolId();
      const docs = await getFirestoreSchoolScopedDocs("attendance", schoolId);
      const rows = docs
        .map((item) => mapFirebaseAttendanceRow(item.id, item.data))
      ...
      return rows.filter((row) => {
        ...
        if (filters?.studentIds?.length && !filters.studentIds.includes(row.student_id)) return false;
        return true;
      });
    } catch (error) {
      if (!isFirebasePermissionError(error) && !isFirebaseQuotaError(error)) {
        throw error;
      }
    }
  }
...
const getAttendanceRowById = async (attendanceId: string): Promise<AttendanceRow> => {
  if (firebaseDb) {
    try {
      const snapshot = await getFirestoreDocument("attendance", attendanceId);
      if (snapshot?.data) {
        return mapFirebaseAttendanceRow(snapshot.id, snapshot.data);
      }
    } catch (error) {
      if (!isFirebasePermissionError(error) && !isFirebaseQuotaError(error)) {
        throw error;
...
const listExistingAttendanceRows = async (
  ...
  if (firebaseDb) {
    try {
      ...
      return docs
        .map((item) => mapFirebaseAttendanceRow(item.id, item.data))
        .filter((row) => row.date === date && row.subject_id === subjectId && studentIds.includes(row.student_id));
    } catch (error) {
      if (!isFirebasePermissionError(error) && !isFirebaseQuotaError(error)) {
        throw error;
      }
...
const listExamRows = async (): Promise<ExamRow[]> => {
  if (firebaseDb) {
    ...
    return docs
      .map((item) => mapFirebaseExamRow(item.id, item.data))
      .sort(
        (left, right) =>
          String(left.start_date ?? left.date ?? left.exam_date ?? "").localeCompare(
            String(right.start_date ?? right.date ?? right.exam_date ?? ""),
          ),
      );
  }
...
const getExamRowById = async (examId: string): Promise<ExamRow> => {
  if (firebaseDb) {
    const snapshot = await getFirestoreDocument("exams", examId);
    if (!snapshot?.data) {
      throw new Error("Exam not found.");
    }
    return mapFirebaseExamRow(snapshot.id, snapshot.data);
  }
...
const listExamSubjectRows = async (filters?: { examId?: string }): Promise<ExamSubjectRow[]> => {
  if (firebaseDb) {
    ...
    return docs
      .map((item) => mapFirebaseExamSubjectRow(item.id, item.data))
      .filter((row) => (filters?.examId ? row.exam_id === filters.examId : true));
  }
  ...
  if (firebaseDb) {
    ...
    await setFirestoreDocumentThroughServer(
      ...
      examId,
      {
        ...
        examDate: summary.startDate,
        exam_date: summary.startDate,
        examSession: summary.examSession,
  ...
  if (filters?.className || filters?.section || filters?.studentId) {
    ...
    const allowedIds = new Set(
      students
        .filter(
          (student) =>
            (!filters.className || normalizeComparableText(student.className) === normalizeComparableText(filters.className)) &&
            (!filters.section || normalizeComparableText(student.section) === normalizeComparableText(filters.section)) &&
...
export const listAttendanceSubjectsFromTimetable = async (
  ...
  if (firebaseDb) {
    const timetableSlots = await listTimetableSlots({ className, section });
    return Array.from(
      new Map(
        timetableSlots
          .filter((slot) => !slot.isBreak && !slot.isCancelled && Boolean(slot.subjectId))
          .map((slot) => [
            `${slot.subjectId}-${slot.teacherId ?? "none"}`,
            {
              subjectId: slot.subjectId!,
...
export const loadAttendanceRoster = async (values: AttendanceFormValues): Promise<AttendanceSession> => {
  ...
  let matchedStudents: StudentRecord[] = [];

  if (firebaseDb) {
    const schoolId = requireCurrentSchoolId();
    const studentRows = await getFirestoreStudentsByClassSection(schoolId, values.className, values.section);
    const usersMap = await getUsersMap(studentRows.map((item) => item.user_id));
    const parentsMap = await getParentsMap(
      studentRows.map((item) => item.parent_id).filter((value): value is string => Boolean(value)),
    );
    ...
    const studentRows = await listStudents();
    matchedStudents = studentRows
      .filter(
        (student) =>
          normalizeComparableText(student.className) === normalizeComparableText(values.className) &&
          normalizeComparableText(student.section) === normalizeComparableText(values.section),
      )
      .sort((left, right) => (left.name ?? "").localeCompare(right.name ?? ""));
  }
  ...
  const rows: AttendanceRosterRow[] = matchedStudents.map((student) => ({
    studentId: student.id,
    studentCode: student.studentCode ?? student.schoolId ?? null,
...
export const saveAttendanceSession = async (session: AttendanceSession): Promise<AttendanceSession> => {
  ...
  const userId = activeSession?.uid ?? authStore.getState().user?.id ?? null;
  ...
  invalidateSchoolScopedCacheByPrefix(["attendance:"]);
...
export const deleteAttendanceSession = async (
  ...
  if (firebaseDb) {
    const schoolId = requireCurrentSchoolId();
    const studentRows = await getFirestoreStudentsByClassSection(schoolId, className, section);
    const studentIds = studentRows.map((row) => row.id);
    if (studentIds.length === 0) return;

    const existingRows = await listExistingAttendanceRows(date, subjectId, studentIds);
    await Promise.all(existingRows.map((row) => deleteFirestoreDocumentThroughServer("attendance", row.id, schoolId)));
    invalidateSchoolScopedCacheByPrefix(["attendance:"]);
...
export const listExams = async (): Promise<ExamRecord[]> => {
  ...
  if (firebaseDb) {
    const examSubjectRows = await listExamSubjectRows();
    const countMap = new Map<string, number>();
    examSubjectRows.forEach((row) => {
...
export const saveExamSubjects = async (
  ...
  if (firebaseDb) {
    const schoolId = requireCurrentSchoolId();
    const existingDocs = await getFirestoreDocsByFilterVariants(
      "exam_subjects",
      [
        [
          { field: "schoolId", value: schoolId },
          { field: "exam_id", value: examId },
        ],
        [
          { field: "schoolId", value: schoolId },
    ...
    await Promise.all(
      existingDocs.map((doc) => deleteFirestoreDocumentThroughServer("exam_subjects", doc.id, schoolId)),
    );

    await Promise.all(
      normalizedSubjects.map((subject) => {
        const examSubjectId = globalThis.crypto?.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;
        return setFirestoreDocumentThroughServer(
          "exam_subjects",
          examSubjectId,
          {
            schoolId,
    ...
    await syncExamSummaryFromSubjects(
      ...
      normalizedSubjects.map((subject, index) => ({
        id: `${examId}-${subject.subjectId}-${index}`,
        exam_id: examId,
        subject_id: subject.subjectId,
        max_marks: subject.maxMarks,
        exam_date: subject.examDate,
  ...
  if (
    ...
    throw new Error("Unable to save the exam subject schedule.");
  }
...
const invalidateExamDataCache = () => {
  invalidateSchoolScopedCache();
  invalidateFirebaseCache(["admin:marks:"]);
};
...
export const createExam = async (values: ExamFormValues) => {
  ...
  await ensureUniqueExamEntries(values, startDate);
  ...
  if (firebaseDb) {
    const schoolId = requireCurrentSchoolId();
    const createdAt = new Date().toISOString();
    const createdRows: ExamRow[] = [];

    for (const item of payload) {
      const examId = globalThis.crypto?.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;
      await setFirestoreDocumentThroughServer(
        "exams",
        examId,
        {
    ...
    const exam = mapExam(createdRows[0]);
...
export const updateExam = async (examId: string, values: ExamFormValues) => {
  ...
  const endDate = derivedSummary?.endDate ?? values.endDate;
  const examSession = derivedSummary?.examSession ?? values.examSession;
  await ensureUniqueExamEntries(values, startDate, examId);
  const basePayload = {
    name: values.name,
    class: values.className,
    section: values.section,
...
export const deleteExam = async (examId: string) => {
  if (firebaseDb) {
    const schoolId = requireCurrentSchoolId();
    const examSubjectDocs = await getFirestoreDocsByFilterVariants(
      "exam_subjects",
      [
        [
          { field: "schoolId", value: schoolId },
    ...
    const resultDocs = await getFirestoreDocsByFilterVariants(
      "results",
      [
        [
          { field: "schoolId", value: schoolId },
    ...
    const markDocs = await getFirestoreDocsByFilterVariants(
      "marks",
      [
        [
          { field: "schoolId", value: schoolId },
    ...
    await Promise.all(
      examSubjectDocs.map((doc) => deleteFirestoreDocumentThroughServer("exam_subjects", doc.id, schoolId)),
    );
    await Promise.all(resultDocs.map((doc) => deleteFirestoreDocumentThroughServer("results", doc.id, schoolId)));
    await Promise.all(markDocs.map((doc) => deleteFirestoreDocumentThroughServer("marks", doc.id, schoolId)));
    await deleteFirestoreDocumentThroughServer("exams", examId, schoolId);
  ...
  const client = requireDatabaseClient();
  const deleteResults = await client.from("results").delete().eq("exam_id", examId);
  if (deleteResults.error) throw new Error(deleteResults.error.message);

  const deleteMarks = await client.from("marks").delete().eq("exam_id", examId);
  if (deleteMarks.error) throw new Error(deleteMarks.error.message);

  const deleteSubjects = await client.from("exam_subjects").delete().eq("exam_id", examId);
  if (deleteSubjects.error) throw new Error(deleteSubjects.error.message);
...
export const listResults = async (examId?: string): Promise<ResultRecord[]> => {
  if (firebaseDb) {
    ...
    const marks = markDocs
      .map((doc) => ({
        id: doc.id,
        exam_id: String(doc.data.examId ?? doc.data.exam_id ?? ""),
        student_id: String(doc.data.studentId ?? doc.data.student_id ?? ""),
        subject_id: String(doc.data.subjectId ?? doc.data.subject_id ?? ""),
        marks_obtained: getFirebaseNumber(doc.data, ["marksObtained", "marks_obtained"]),
        grade: getFirebaseString(doc.data, ["grade"]),
      }))
      .filter((row) => row.exam_id && row.student_id && row.subject_id);
    ...
    const examSubjectMap = new Map(
      examSubjectDocs
        .map((doc) => ({
          exam_id: String(doc.data.examId ?? doc.data.exam_id ?? ""),
          subject_id: String(doc.data.subjectId ?? doc.data.subject_id ?? ""),
          max_marks: getFirebaseNumber(doc.data, ["maxMarks", "max_marks"]) ?? 100,
        }))
        .filter((row) => row.exam_id && row.subject_id)
        .map((row) => [`${row.exam_id}:${row.subject_id}`, Number(row.max_marks ?? 100)] as const),
    );
    ...
    marks.forEach((row) => {
      ...
      const current = grouped.get(key) ?? {
        id: key,
        studentId: row.student_id,
        studentCode: student.schoolId ?? student.studentCode ?? null,
...
export const getResultDetail = async (resultId: string): Promise<ResultDetail> => {
  if (firebaseDb) {
    ...
    if (resultId.includes(":")) {
      ...
      const markDoc = await getFirestoreDocumentThroughServer("marks", resultId, schoolId);
      if (!markDoc) throw new Error("Result not found.");
      studentId = String(markDoc.data.studentId ?? markDoc.data.student_id ?? "");
      examId = String(markDoc.data.examId ?? markDoc.data.exam_id ?? "");
    }
...
export const loadExamMarksSession = async (
  ...
  let rows: ExamMarksRow[] = [];

  if (firebaseDb) {
    const schoolId = requireCurrentSchoolId();
    const studentRows = await getFirestoreStudentsByClassSection(schoolId, exam.className, exam.section);
    const markDocs = await getFirestoreDocsByFilterVariants(
      "marks",
      [
        [
    ...
    const resultMap = new Map(
      markDocs.map((doc) => {
        ...
        return [
          ...
          {
            id: doc.id,
            marks_obtained: getFirebaseNumber(data, ["marksObtained", "marks_obtained"]),
          },
        ] as const;
      }),
    );
    ...
    rows = (studentRows ?? [])
      .sort((left: { name: string | null }, right: { name: string | null }) => String(left.name ?? "").localeCompare(String(right.name ?? "")))
      .map((student: { id: string; name: string | null; student_code: string | null }) => {
        const existing = resultMap.get(student.id);
        const marks = existing?.marks_obtained != null ? String(existing.marks_obtained) : "";
        return {
          studentId: student.id,
  ...
  if (firebaseDb) {
    ...
    const existingDocs = await getFirestoreDocsByFilterVariants(
      ...
      [
        [
          { field: "schoolId", value: schoolId },
          { field: "examId", value: session.examId },
          { field: "subjectId", value: session.subjectId },
        ],
        [
          { field: "schoolId", value: schoolId },
          { field: "exam_id", value: session.examId },
          { field: "subject_id", value: session.subjectId },
        ],
        [
          { field: "school_id", value: schoolId },
          { field: "examId", value: session.examId },
    ...
    const existingMap = new Map(
      existingDocs.map((doc) => {
        const studentId = String(doc.data.studentId ?? doc.data.student_id ?? "");
        return [studentId, doc.id] as const;
      }),
    );
    ...
    for (const row of rowsToSave) {
      ...
      const existingId = existingMap.get(row.studentId);
      if (existingId) {
        await setFirestoreDocumentThroughServer("marks", existingId, payload, { schoolId, merge: true });
      } else {
        const markId = globalThis.crypto?.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;
        await setFirestoreDocumentThroughServer("marks", markId, payload, { schoolId });
      }
    }
    invalidateFirebaseCache(["admin:marks:"]);
  } else {
...
export const createFee = async (values: FeeFormValues) => {
  ...
  await ensureUniqueFeeEntry(values.studentId, values.dueDate);
  const studentsMap = await getStudentsMap([values.studentId]);
  ...
  if (!modernInsert.error && modernInsert.data) {
    const row = modernInsert.data as FeeRow;
    const student = studentsMap.get(row.student_id);
    const fee = {
      id: row.id,
      studentId: row.student_id,
      ...
      remainingAmount: Math.max(Number(row.total_amount ?? 0) - Number(row.paid_amount ?? 0), 0),
      status: row.status ?? "Unpaid",
      dueDate: row.due_date,
    };
    queueAuditEvent("CREATE", "FEES", fee.id);
    return fee;
  }
  ...
  if (
    ...
    const row = legacyInsert.data as FeeRow;
    const student = studentsMap.get(row.student_id);
    const total = Number(row.amount ?? 0);
    const paid = row.status?.toLowerCase() === "paid" ? total : 0;
    const fee = {
      id: row.id,
      studentId: row.student_id,
      studentName: student?.name ?? "Unknown student",
      ...
      status: row.status ?? deriveFeeStatus(total, paid),
      dueDate: row.due_date,
    };
    queueAuditEvent("CREATE", "FEES", fee.id);
    return fee;
...
export const updateFee = async (feeId: string, values: FeeFormValues) => {
  ...
  const totalAmount = Number(values.totalAmount);
  await ensureUniqueFeeEntry(values.studentId, values.dueDate, feeId);
  ...
  if (
    ...
    const student = await getStudentDetail(values.studentId).catch(() => null);
    const row = legacyUpdate.data as FeeRow;
    const total = Number(row.amount ?? totalAmount);
    ...
    queueAuditEvent("UPDATE", "FEES", fee.id);
    return fee;
  }

  if (modernUpdate.error) throw new Error(modernUpdate.error.message);
  const student = await getStudentDetail(values.studentId).catch(() => null);
  const row = modernUpdate.data as FeeRow;
  const fee = {
    id: feeId,
    studentId: values.studentId,
    studentName: student?.name ?? current.studentName,
    ...
    paidAmount: Number(row.paid_amount ?? paidAmount),
    remainingAmount: Math.max(Number(row.total_amount ?? totalAmount) - Number(row.paid_amount ?? paidAmount), 0),
...
export const listLeaves = async (): Promise<LeaveRecord[]> => {
  if (firebaseDb) {
    return readSchoolScopedCache("leaves", async () => {
      const schoolId = requireCurrentSchoolId();
      const rows = (await getFirestoreSchoolScopedDocs("leaves", schoolId))
        .map((item) => mapFirebaseLeaveRow(item.id, item.data))
        .sort((left, right) => right.start_date.localeCompare(left.start_date));
      const staffMap = new Map((await listStaff()).map((employee) => [employee.id, employee]));

      return rows.map((row) => {
        const staffId = row.staff_id ?? row.employee_id ?? null;
        const employee = staffId ? staffMap.get(staffId) ?? null : null;
        return {
          id: row.id,
          staffId,
...
export const createLeave = async (values: LeaveFormValues) => {
  if (firebaseDb) {
    const leaveId = globalThis.crypto?.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;
    const schoolId = requireCurrentSchoolId();
    const payload = {
      schoolId,
      school_id: schoolId,
    ...
    try {
      await firebaseSetDoc(firebaseDoc(firebaseDb, "leaves", leaveId), payload);
    } catch (error) {
      if (isFirebaseQuotaError(error)) {
        throw error;
      }
      ...
      await setFirestoreDocumentThroughServer("leaves", leaveId, payload, { schoolId });
    }

    invalidateSchoolScopedCache(["leaves"]);
...
export const updateLeave = async (leaveId: string, values: LeaveFormValues) => {
  if (firebaseDb) {
    ...
    try {
      await firebaseUpdateDoc(firebaseDoc(firebaseDb, "leaves", leaveId), payload);
    } catch (error) {
      if (isFirebaseQuotaError(error)) {
        throw error;
      }
      ...
      await setFirestoreDocumentThroughServer("leaves", leaveId, payload, { schoolId, merge: true });
    }
...
export const deleteLeave = async (leaveId: string) => {
  if (firebaseDb) {
    const schoolId = requireCurrentSchoolId();
    try {
      await firebaseDeleteDoc(firebaseDoc(firebaseDb, "leaves", leaveId));
    } catch (error) {
      if (isFirebaseQuotaError(error)) {
        throw error;
      }
      ...
      await deleteFirestoreDocumentThroughServer("leaves", leaveId, schoolId);
    }
    invalidateSchoolScopedCache(["leaves"]);
...
const syncApprovedLeaveImpacts = async (leaveId: string) => {
  ...
  invalidateSchoolScopedCacheByPrefix(["timetable:", "attendance:", "notifications:"]);
  return impacts;
};
...
export const reviewLeaveByHr = async (
  ...
  if (firebaseDb) {
    const snapshot = await getFirestoreDocument("leaves", leaveId);
    if (!snapshot) {
      throw new Error("Leave request not found.");
    }

    const leaveRow = mapFirebaseLeaveRow(snapshot.id, snapshot.data);
    const currentStatus = leaveRow.status as LeaveRecord["status"];
    if (currentStatus !== "Pending_HR") {
      throw new Error("Only HR-stage leave requests can be reviewed here.");
    }
...
export const reviewLeaveByAdmin = async (
  ...
  if (firebaseDb) {
    ...
    if (currentStatus !== "Pending_Admin") {
      throw new Error("Only leadership-stage leave requests can be reviewed here.");
    }
    ...
    try {
      ...
      await setFirestoreDocumentThroughServer("leaves", leaveId, payload, {
    ...
    invalidateSchoolScopedCache(["leaves"]);
    invalidateSchoolScopedCacheByPrefix(["notifications:", "timetable:", "attendance:"]);

    if (decision === "Approved" && databaseClient) {
...
export const getAnalyticsDashboard = async (): Promise<AnalyticsDashboard> => {
  if (firebaseDb) {
    const [students, staff, attendance, results] = await Promise.all([
      listStudents(),
      listStaff(),
      listAttendance(),
      listResults(),
    ...
    const presentCount = attendance.filter((row) => String(row.status ?? "").toLowerCase() === "present").length;
    const totalAttendance = attendance.length;
    const attendanceGrouped = Array.from(
      attendance.reduce((map, row) => {
    ...
    const subjectPerformanceMap = results.reduce((map, row) => {
      const key = row.examName ?? "Unknown Exam";
      map.set(key, [...(map.get(key) ?? []), Number(row.percentage ?? 0)]);
      return map;
    }, new Map<string, number[]>());
      ...
      metrics: [
        {
          ...
          helper: "Live student count from Firebase records.",
        },
        {
          label: "Total Staff",
          value: String(staff.length),
          helper: "Live staff count from Firebase records.",
        },
        {
          label: "Attendance Logged",
          value: String(totalAttendance),
          helper: "Attendance entries available for this school.",
        },
        {
          label: "Present Rate",
          value: `${Math.round(totalAttendance > 0 ? (presentCount / totalAttendance) * 100 : 0)}%`,
          helper: "Calculated from attendance records already synced to Firebase.",
        },
      ],
      monthlyFees: [],
      attendance: attendanceGrouped.map(([label, value]) => ({
        label,
        value,
        percentage: totalAttendance > 0 ? Math.round((value / totalAttendance) * 100) : 0,
      })),
      performance: Array.from(subjectPerformanceMap.entries()).map(([label, values]) => {
        const validValues = values.filter((entry) => Number.isFinite(entry));
        const total = validValues.reduce((sum, entry) => sum + entry, 0);
        return {
          label,
          value: Math.round(validValues.length > 0 ? total / validValues.length : 0),
        };
      }),
    };
  }
...
export const listAuditLogs = async (filters?: {
  ...
  const cacheKey = getDataCacheKey(`audit:${filters?.userId ?? "all"}:${filters?.module ?? "all"}:${filters?.date ?? "all"}`);
  const cached = dataStore.getState().getCachedPage<AuditLogRecord>("auditLogsCache", cacheKey);
  ...
  const schoolId = requireCurrentSchoolId();
  if (firebaseDb) {
    const docs = await getFirestoreSchoolScopedDocs("auditLogs", schoolId);
    const rows = docs
      .map((doc) => ({
        id: doc.id,
        school_id: getFirebaseString(doc.data, ["schoolId", "school_id"]),
        user_id: getFirebaseString(doc.data, ["userId", "user_id"]),
        action: getFirebaseString(doc.data, ["action"]) ?? "UNKNOWN",
        module: getFirebaseString(doc.data, ["module"]) ?? "UNKNOWN",
        record_id: getFirebaseString(doc.data, ["recordId", "record_id"]),
        created_at: getFirebaseString(doc.data, ["createdAt", "created_at"]),
      }))
      .filter((row) => {
        if (filters?.userId && row.user_id !== filters.userId) return false;
        if (filters?.module && row.module !== filters.module) return false;
        if (filters?.date) {
          return String(row.created_at ?? "").startsWith(filters.date);
        }
        return true;
      })
      .sort((left, right) => String(right.created_at ?? "").localeCompare(String(left.created_at ?? "")));
    ...
    dataStore.getState().setPageCache("auditLogsCache", cacheKey, { items: records, nextCursor: null, loadedAt: Date.now() });
    return records;
  }
...
export const createSalary = async (values: SalaryFormValues) => {
  const client = requireDatabaseClient();
  await ensureUniqueSalaryEntry(values.staffId, values.month);
  const staff = await getStaffDetail(values.staffId).catch(() => null);
  const modernInsert = await client
    .from("salary")
    .insert({
      staff_id: values.staffId,
    ...
    .single();
  if (!modernInsert.error && modernInsert.data) {
    const row = modernInsert.data as SalaryRow;
    const salary = {
      id: row.id,
      staffId: row.staff_id ?? values.staffId,
    ...
    queueAuditEvent("CREATE", "SALARY", salary.id);
    return salary;
  }
  if (modernInsert.error && isMissingColumnError(modernInsert.error.message, "salary.staff_id")) {
    ...
    const row = legacyInsert.data as SalaryRow;
    const salary = {
      id: row.id,
      staffId: row.employee_id ?? values.staffId,
      employeeName: staff?.name ?? "Unknown employee",
      role: staff?.role ?? null,
      amount: Number(row.amount),
      month: row.month,
...
export const updateSalary = async (salaryId: string, values: SalaryFormValues) => {
  ...
  await ensureUniqueSalaryEntry(values.staffId, values.month, salaryId);
  const staff = await getStaffDetail(values.staffId).catch(() => null);
  const modernUpdate = await client
    .from("salary")
    .update({
  ...
  if (modernUpdate.error && isMissingColumnError(modernUpdate.error.message, "salary.staff_id")) {
    const legacyUpdate = await client
      ...
      .single();
    if (legacyUpdate.error) throw new Error(legacyUpdate.error.message);
    const row = legacyUpdate.data as SalaryRow;
    const salary = {
      id: salaryId,
      staffId: row.employee_id ?? values.staffId,
      employeeName: staff?.name ?? existing.employeeName,
      role: staff?.role ?? existing.role,
      amount: Number(row.amount),
      month: row.month,
      status: row.status,
    };
    queueAuditEvent("UPDATE", "SALARY", salary.id);
    return salary;
  }
  if (modernUpdate.error) throw new Error(modernUpdate.error.message);
  const row = modernUpdate.data as SalaryRow;
  const salary = {
    id: salaryId,
    staffId: row.staff_id ?? values.staffId,
    employeeName: staff?.name ?? existing.employeeName,
...
const mapRoute = (row: RouteRow, vehiclesMap: Map<string, VehicleRecord>): RouteRecord => ({
  id: row.id,
...
const mapFirebaseRouteRow = (id: string, data: Record<string, unknown>): RouteRow => ({
  id,
  school_id: getFirebaseString(data, ["schoolId", "school_id"]),
  route_name: getFirebaseString(data, ["routeName", "route_name"]) ?? "",
  stops: getFirebaseString(data, ["stops"]) ?? "",
  vehicle_id: getFirebaseString(data, ["vehicleId", "vehicle_id"]),
});

const mapFirebaseVehicleRow = (id: string, data: Record<string, unknown>): VehicleRow => ({
  id,
  school_id: getFirebaseString(data, ["schoolId", "school_id"]),
  vehicle_name: getFirebaseString(data, ["vehicleName", "vehicle_name"]),
  vehicle_number: getFirebaseString(data, ["vehicleNumber", "vehicle_number", "vehicleName", "vehicle_name"]),
  driver_name: getFirebaseString(data, ["driverName", "driver_name"]) ?? "",
  driver_phone: getFirebaseString(data, ["driverPhone", "driver_phone"]),
  capacity: getFirebaseNumber(data, ["capacity"]) ?? 0,
  status: getFirebaseString(data, ["status"]),
});
...
export const listVehicles = async (): Promise<VehicleRecord[]> => {
  if (firebaseDb) {
    const schoolId = requireCurrentSchoolId();
    const rows = (await getFirestoreSchoolScopedDocs("vehicles", schoolId))
      .map((item) => mapFirebaseVehicleRow(item.id, item.data))
      .sort((left, right) => String(left.vehicle_number ?? left.vehicle_name ?? "").localeCompare(String(right.vehicle_number ?? right.vehicle_name ?? "")));
...
export const getVehicleDetail = async (vehicleId: string): Promise<VehicleDetail> => {
  if (firebaseDb) {
    const snapshot = await getFirestoreDocumentThroughServer("vehicles", vehicleId, requireCurrentSchoolId());
    if (!snapshot) throw new Error("Vehicle not found.");

    const vehicle = mapVehicle(mapFirebaseVehicleRow(snapshot.id, snapshot.data));
...
export const createVehicle = async (values: VehicleFormValues) => {
  const vehicleNumber = formatVehicleNumber(values.vehicleName);
  const driverName = formatDriverName(values.driverName);
  if (!isValidVehicleNumber(vehicleNumber)) {
    throw new Error("Vehicle number must be in the format AP 15 GS 2005.");
  }
  if (!driverName) {
    throw new Error("Driver name is required.");
  }
  await ensureUniqueVehicleEntry(vehicleNumber, values.driverPhone);
  if (firebaseDb) {
    const schoolId = requireCurrentSchoolId();
    const vehicleId = globalThis.crypto?.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;
    await setFirestoreDocumentThroughServer(
      "vehicles",
      vehicleId,
      {
    ...
    invalidateSchoolScopedCache(["vehicles", "routes"]);
  ...
  const vehicle = mapVehicle({ ...(fallback.data as VehicleRow), vehicle_name: vehicleNumber, vehicle_number: vehicleNumber, driver_phone: values.driverPhone || null, status: values.status || "Active" });
  invalidateSchoolScopedCache(["vehicles", "routes"]);
  await logAuditEvent("CREATE", "VEHICLE", vehicle.id);
  return vehicle;
};
...
export const updateVehicle = async (vehicleId: string, values: VehicleFormValues) => {
  ...
  await ensureUniqueVehicleEntry(vehicleNumber, values.driverPhone, vehicleId);
  if (firebaseDb) {
    const schoolId = requireCurrentSchoolId();
    await setFirestoreDocumentThroughServer(
      "vehicles",
  ...
  const vehicle = mapVehicle({ ...(fallback.data as VehicleRow), vehicle_name: vehicleNumber, vehicle_number: vehicleNumber, driver_name: driverName, driver_phone: values.driverPhone || null, status: values.status || "Active" });
  invalidateSchoolScopedCache(["vehicles", "routes"]);
  await logAuditEvent("UPDATE", "VEHICLE", vehicle.id);
  return vehicle;
};
...
export const deleteVehicle = async (vehicleId: string) => {
  if (firebaseDb) {
    const schoolId = requireCurrentSchoolId();
    await deleteFirestoreDocumentThroughServer("vehicles", vehicleId, schoolId);
    invalidateSchoolScopedCache(["vehicles", "routes"]);
...
export const listTransportRoutes = async (): Promise<RouteRecord[]> => {
  if (firebaseDb) {
    const schoolId = requireCurrentSchoolId();
    const rows = (await getFirestoreSchoolScopedDocs("routes", schoolId))
      .map((item) => mapFirebaseRouteRow(item.id, item.data))
      .sort((left, right) => String(left.route_name ?? "").localeCompare(String(right.route_name ?? "")));
    const vehiclesMap = new Map((await listVehicles()).map((vehicle) => [vehicle.id, vehicle]));
    return rows.map((row) => mapRoute(row, vehiclesMap));
  }
...
export const createRoute = async (values: RouteFormValues) => {
  await ensureUniqueRouteEntry(values.routeName);
  if (firebaseDb) {
    const schoolId = requireCurrentSchoolId();
    const routeId = globalThis.crypto?.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;
    const routePayload = {
      schoolId,
      routeName: values.routeName.trim(),
      route_name: values.routeName.trim(),
      stops: values.stops.trim(),
    ...
    try {
      await firebaseSetDoc(firebaseDoc(firebaseDb, "routes", routeId), routePayload);
    } catch (error) {
      if (isFirebaseQuotaError(error)) {
        throw error;
      }
      ...
      await setFirestoreDocumentThroughServer("routes", routeId, routePayload, { schoolId });
    }

    invalidateSchoolScopedCache(["routes"]);
...
export const updateRoute = async (routeId: string, values: RouteFormValues) => {
  await ensureUniqueRouteEntry(values.routeName, routeId);
  if (firebaseDb) {
    const schoolId = requireCurrentSchoolId();
    const snapshot = await getFirestoreDocument("routes", routeId);
    if (!snapshot) {
      throw new Error("Route not found.");
    }

    const routeSchoolId = getFirebaseString(snapshot.data, ["schoolId", "school_id"]);
    ...
    try {
      await firebaseUpdateDoc(firebaseDoc(firebaseDb, "routes", routeId), routePayload);
    } catch (error) {
      if (isFirebaseQuotaError(error)) {
        throw error;
      }
      ...
      await setFirestoreDocumentThroughServer("routes", routeId, routePayload, { schoolId, merge: true });
    }
...
export const deleteRoute = async (routeId: string) => {
  if (firebaseDb) {
    const schoolId = requireCurrentSchoolId();
    await deleteFirestoreDocumentThroughServer("routes", routeId, schoolId);
    invalidateSchoolScopedCache(["routes"]);
...
export const listApplicants = async (): Promise<ApplicantRecord[]> => {
  const cacheKey = getDataCacheKey("applicants:all");
  const cached = dataStore.getState().getCachedPage<ApplicantRecord>("applicantsCache", cacheKey);
  if (cached?.items?.length) {
    return cached.items;
  }

  if (firebaseDb) {
    return readSchoolScopedCache("applicants", async () => {
      const schoolId = requireCurrentSchoolId();
      const rows = (await getFirestoreSchoolScopedDocs("applicants", schoolId))
        .map((item) => mapFirebaseApplicantRow(item.id, item.data))
        .sort((left, right) => String(right.created_at ?? "").localeCompare(String(left.created_at ?? "")));
      const records = rows.map(mapApplicant);
      dataStore.getState().setPageCache("applicantsCache", cacheKey, { items: records, nextCursor: null, loadedAt: Date.now() });
      return records;
    });
  }
  ...
  const { data, error } = await client
    ...
    .order("created_at", { ascending: false });
  if (!error) {
    const records = ((data ?? []) as ApplicantRow[]).map(mapApplicant);
    dataStore.getState().setPageCache("applicantsCache", cacheKey, { items: records, nextCursor: null, loadedAt: Date.now() });
    return records;
  }
  const fallback = await client
    ...
    .order("created_at", { ascending: false });
  if (fallback.error) throw new Error(fallback.error.message);
  const records = ((fallback.data ?? []) as ApplicantRow[]).map(mapApplicant);
  dataStore.getState().setPageCache("applicantsCache", cacheKey, { items: records, nextCursor: null, loadedAt: Date.now() });
  return records;
};
...
export const createApplicant = async (values: ApplicantFormValues) => {
  await ensureUniqueApplicantEntry(values);
  const email = buildApplicantEmail(values);
  const status = normalizeApplicantStatus(values.status);
  const schoolId = requireCurrentSchoolId();

  if (firebaseDb) {
    const applicantId = globalThis.crypto?.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;
    const createdAt = new Date().toISOString();
    ...
    try {
      await firebaseSetDoc(firebaseDoc(firebaseDb, "applicants", applicantId), applicantPayload);
    } catch (error) {
      if (isFirebaseQuotaError(error)) {
        throw error;
      }
      ...
      await setFirestoreDocumentThroughServer("applicants", applicantId, applicantPayload, { schoolId });
    }

    invalidateSchoolScopedCache(["applicants"]);
...
export const updateApplicant = async (applicantId: string, values: ApplicantFormValues) => {
  await ensureUniqueApplicantEntry(values, applicantId);
  const email = buildApplicantEmail(values);
  const status = normalizeApplicantStatus(values.status);

  if (firebaseDb) {
    const snapshot = await getFirestoreDocument("applicants", applicantId);
    if (!snapshot) {
      throw new Error("Applicant not found.");
    }

    const schoolId = requireCurrentSchoolId();
    const applicantSchoolId = getFirebaseString(snapshot.data, ["schoolId", "school_id"]);
    if (applicantSchoolId !== schoolId) {
      throw new Error("Applicant not found.");
    }
    ...
    try {
      await firebaseUpdateDoc(firebaseDoc(firebaseDb, "applicants", applicantId), applicantPayload);
    } catch (error) {
      if (isFirebaseQuotaError(error)) {
        throw error;
      }
      ...
      await setFirestoreDocumentThroughServer("applicants", applicantId, applicantPayload, { schoolId, merge: true });
    }
    ...
    const applicant = mapApplicant({
      ...
      parent_email: values.parentEmail || null,
      parent_phone: values.parentPhone || null,
      created_at: snapshot.data.createdAt ? String(snapshot.data.createdAt) : null,
    });
...
export const deleteApplicant = async (applicantId: string) => {
  if (firebaseDb) {
    ...
    try {
      await firebaseDeleteDoc(firebaseDoc(firebaseDb, "applicants", applicantId));
    } catch (error) {
      if (isFirebaseQuotaError(error)) {
        throw error;
      }
      ...
      await deleteFirestoreDocumentThroughServer("applicants", applicantId, schoolId);
    }
...
export const approveApplicant = async (values: ApplicantApprovalValues) => {
  const applicant = await invokeAdminAction<ApplicantRecord>("approve_applicant", values);
...
export const rejectApplicant = async (applicantId: string) => {
  if (firebaseDb) {
    ...
    try {
      await firebaseUpdateDoc(firebaseDoc(firebaseDb, "applicants", applicantId), payload);
    } catch (error) {
      if (isFirebaseQuotaError(error)) {
        throw error;
      }
      ...
      await setFirestoreDocumentThroughServer("applicants", applicantId, payload, {
        schoolId: requireCurrentSchoolId(),
        merge: true,
      });
    }
    ...
    invalidateSchoolScopedCache(["applicants"]);
    const applicant = (await listApplicants()).find((item) => item.id === applicantId);
    if (!applicant) {
      throw new Error("Applicant not found.");
    }
    await logAuditEvent("UPDATE", "APPLICANT", applicant.id);
...
export const getDashboardOverview = async (
  ...
  const cacheKey = getDataCacheKey("dashboard-overview");
  const cachedEntry = dataStore.getState().dashboardStatsCache[cacheKey];
  const cached = cachedEntry?.value as DashboardOverview | undefined;
  if (!options?.forceRefresh && cached) {
    if (!isPersistedCacheFresh(cachedEntry?.loadedAt)) {
      void scheduleBackgroundRefresh(cacheKey, () => getDashboardOverview({ forceRefresh: true }));
    }
    return cached;
  }

  try {
    const overview = await invokeServerAction<DashboardOverview>("get_dashboard_fast", {});
    dataStore.getState().setDashboardStats(cacheKey, overview);
    return overview;
  } catch (error) {
    if (!isAdminApiUnavailable(error)) {
      throw error;
...
export const preloadStudents = async () => {
  await listStudents({ forceRefresh: true });
};

export const preloadStaff = async () => {
  await listStaff({ forceRefresh: true });
};

export const preloadDashboardStats = async () => {
  await getDashboardOverview({ forceRefresh: true });
};

export const preloadCoreDashboardData = async (role: string | null) => {
  if (!role) return;

  if (role === ROLES.ADMIN || role === ROLES.STAFF || role === ROLES.PRINCIPAL) {
    await Promise.all([
      preloadStudents(),
      preloadStaff(),
...
export const listStudentsFast = async (
  limitCount = 20,
  cursor?: string | null,
): Promise<FastListPage<{ id: string; name: string; className: string | null; section: string | null; createdAt: string | null }>> =>
  invokeServerAction("get_students_fast", {
    limit: limitCount,
    cursor: cursor ?? "",
  });

export const listStaffFast = async (
  limitCount = 20,
  cursor?: string | null,
): Promise<FastListPage<{ id: string; name: string; role: string; createdAt: string | null }>> =>
  invokeServerAction("get_staff_fast", {
    limit: limitCount,
    cursor: cursor ?? "",
  });
...
const getTimetableRowById = async (slotId: string): Promise<TimetableRow> => {
  if (firebaseDb) {
    const snapshot = await getFirestoreDocument("timetable", slotId);
    if (!snapshot) {
      throw new Error("Timetable slot not found.");
    }
    return mapFirebaseTimetableRow(snapshot.id, snapshot.data);
  }
...
const saveTimetableRow = async (
  ...
  if (firebaseDb) {
    ...
    try {
      if (options?.merge) {
        await firebaseUpdateDoc(firebaseDoc(firebaseDb, "timetable", rowId), payload);
      } else {
        await firebaseSetDoc(firebaseDoc(firebaseDb, "timetable", rowId), payload);
      }
    } catch (error) {
      if (isFirebaseQuotaError(error)) {
        throw error;
      ...
      await setFirestoreDocumentThroughServer("timetable", rowId, payload, {
        schoolId,
        merge: Boolean(options?.merge),
      });
    }

    invalidateSchoolScopedCacheByPrefix(["timetable:", "attendance:"]);
    return;
  }

  const client = requireDatabaseClient();
  const query = options?.merge
    ? client.from("timetable").update(payload).eq("id", rowId)
    : client.from("timetable").insert({ id: rowId, ...payload });
  const result = await query;
  if (result.error) throw new Error(result.error.message);
};

const deleteTimetableRow = async (rowId: string) => {
  if (firebaseDb) {
    const schoolId = requireCurrentSchoolId();

    try {
      await firebaseDeleteDoc(firebaseDoc(firebaseDb, "timetable", rowId));
    } catch (error) {
      if (isFirebaseQuotaError(error)) {
        throw error;
      }
      ...
      await deleteFirestoreDocumentThroughServer("timetable", rowId, schoolId);
    }
  ...
  const client = requireDatabaseClient();
  const result = await client.from("timetable").delete().eq("id", rowId);
  if (result.error) throw new Error(result.error.message);
};
...
const syncSchoolWideBreakForClassDay = async (
  ...
  if (previousBreakGroup) {
    ...
    if (previousRows.length > 0) {
      for (const row of previousRows) {
        await deleteTimetableRow(row.id);
      }
  ...
  if (existingSameBreak) {
    await saveTimetableRow(
      existingSameBreak.id,
      firebaseDb
        ? {
            breakType: values.breakType,
            break_type: values.breakType,
            breakLabel: values.breakLabel.trim(),
            break_label: values.breakLabel.trim(),
            updatedAt: new Date().toISOString(),
          }
        : { break_type: values.breakType, break_label: values.breakLabel.trim() },
      { merge: true },
    );
    return;
  }
  ...
  const rowId = globalThis.crypto?.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;
  await saveTimetableRow(
    rowId,
    firebaseDb
      ? {
...
const syncSchoolWideBreak = async (
  ...
  const existingBreakTypes = firebaseDb
    ? (await listTimetableSlots()).filter((row) => row.isBreak).map((row) => ({ break_type: row.breakType ?? null }))
    : await (async () => {
        const client = requireDatabaseClient();
        const { data, error } = await client
          .from("timetable")
          ...
          .eq("is_break", true);
        if (error) throw new Error(error.message);
        return (data ?? []) as Array<{ break_type: "Short Break" | "Lunch Break" | null }>;
      })();
...
export const listTimetableClassOptions = async (): Promise<TimetableClassOption[]> => {
  return readSchoolScopedCache("timetable-class-options", async () => {
    const schoolId = requireCurrentSchoolId();
    const cacheKey = getSchoolCacheKey("timetable-class-options", schoolId);
    const now = Date.now();
    const cached = loadTimetableClassOptionsCache.get(cacheKey);
    ...
    const promise = (async () => {
      if (firebaseDb) {
        ...
        if (classMap.size === 0) {
          const [studentRows, staffRows] = await Promise.all([listStudents(), listStaff()]);
          ...
          staffRows.forEach((item) => {
            const className = item.assignedClass?.trim();
            const section = item.assignedSection?.trim();
            if (!className || !section) return;
            classMap.set(className, (classMap.get(className) ?? new Set()).add(section));
          });
        }
        ...
        loadTimetableClassOptionsCache.set(cacheKey, {
          value: options,
      ...
      loadTimetableClassOptionsCache.delete(cacheKey);
      throw error;
    });
  ...
  if (firebaseDb) {
    ...
    return {
      ...
      canEditSelectedClass:
        Boolean(staff.isClassCoordinator) &&
        staff.assignedClass === selectedClass &&
        staff.assignedSection === selectedSection,
      isClassCoordinator: Boolean(staff.isClassCoordinator),
      assignedClass: staff.assignedClass ?? null,
      assignedSection: staff.assignedSection ?? null,
    };
  }
...
export const loadTimetable = async (
  ...
  if (firebaseDb) {
    ...
    const rows = docs
      .map((item) => mapFirebaseTimetableRow(item.id, item.data))
      .sort((left, right) => left.day.localeCompare(right.day) || left.start_time.localeCompare(right.start_time));

    return mapTimetableSlots(rows);
  }
...
export const listTimetableSlots = async (filters?: {
  ...
  if (firebaseDb) {
    const schoolId = requireCurrentSchoolId();
    const docs = await getFirestoreSchoolScopedDocs("timetable", schoolId);
    ...
    const rows = docs
      ...
      .filter((item) => {
        ...
        if (filters?.teacherId && item.teacher_id !== filters.teacherId) return false;
        if (filters?.day && item.day !== filters.day) return false;
        if (!filters?.day && filters?.date && item.day !== dayFromDate(filters.date)) return false;
        return true;
      })
      .sort((left, right) => left.day.localeCompare(right.day) || left.start_time.localeCompare(right.start_time));
...
const checkTimetableConflicts = async (
  ...
  if (firebaseDb) {
    const [teacherRows, classRows] = await Promise.all([
      values.isBreak || !values.teacherId
        ? Promise.resolve([] as TimetableSlotRecord[])
        : listTimetableSlots({ teacherId: values.teacherId, day: values.day }),
      listTimetableSlots({ className: values.className, section: values.section, day: values.day }),
    ]);

    const overlaps = (startA: string, endA: string, startB: string, endB: string) =>
      startA < endB && endA > startB;

    const teacherConflict = teacherRows.find(
      (row) =>
        row.id !== excludeId &&
        overlaps(row.startTime, row.endTime, values.startTime, values.endTime),
    );
    if (teacherConflict) {
      throw new Error("Teacher conflict: this teacher is already assigned to another class at this time.");
    }

    const classConflict = classRows.find(
      (row) =>
        row.id !== excludeId &&
        overlaps(row.startTime, row.endTime, values.startTime, values.endTime),
    );
    if (classConflict?.isBreak) {
      throw new Error(`Break conflict: this time is already reserved for ${classConflict.breakType ?? classConflict.breakLabel ?? "a school break"}.`);
    }
    if (classConflict) {
...
export const createTimetableSlot = async (
  values: TimetableFormValues,
  context?: { userId?: string; role: string | null; timetableSettings?: TimetableSettings; applySchoolWideBreak?: boolean },
) => {
  ...
  if (values.isBreak) {
    if (context?.role === ROLES.ADMIN && context.applySchoolWideBreak) {
  ...
  if (firebaseDb) {
    ...
    const timetablePayload = {
      ...
      breakType: values.isBreak ? values.breakType : null,
      break_type: values.isBreak ? values.breakType : null,
      breakLabel: values.isBreak ? values.breakLabel.trim() : null,
    ...
    try {
      await firebaseSetDoc(firebaseDoc(firebaseDb, "timetable", slotId), timetablePayload);
    } catch (error) {
      if (isFirebaseQuotaError(error)) {
        throw error;
      }
      ...
      await setFirestoreDocumentThroughServer("timetable", slotId, timetablePayload, { schoolId });
    }

    invalidateSchoolScopedCacheByPrefix(["timetable:", "attendance:"]);
    const [slot] = await mapTimetableSlots([mapFirebaseTimetableRow(slotId, timetablePayload)]);
    queueAuditEvent("CREATE", "TIMETABLE", slot.id);
    return slot;
  }
...
export const updateTimetableSlot = async (
  ...
  if (firebaseDb) {
    ...
    const timetablePayload = {
      ...
      class: values.className,
      section: values.section,
      subjectId: values.isBreak ? null : values.subjectId,
      subject_id: values.isBreak ? null : values.subjectId,
      teacherId: values.isBreak ? null : values.teacherId,
      teacher_id: values.isBreak ? null : values.teacherId,
      day: values.day,
    ...
    try {
      await firebaseUpdateDoc(firebaseDoc(firebaseDb, "timetable", slotId), timetablePayload);
    } catch (error) {
      if (isFirebaseQuotaError(error)) {
        throw error;
      }
      ...
      await setFirestoreDocumentThroughServer("timetable", slotId, timetablePayload, { schoolId, merge: true });
    }

    invalidateSchoolScopedCacheByPrefix(["timetable:", "attendance:"]);
    const [slot] = await mapTimetableSlots([
      {
        ...existingRow,
        class: values.className,
        section: values.section,
        ...
        end_time: values.endTime,
        is_break: values.isBreak,
        break_type: values.isBreak ? (values.breakType || null) : null,
        break_label: values.isBreak ? values.breakLabel.trim() : null,
      },
    ]);
    queueAuditEvent("UPDATE", "TIMETABLE", slot.id);
    return slot;
  }
...
export const deleteTimetableSlot = async (
  ...
  if (firebaseDb && !Boolean(existingRow.is_break)) {
    await deleteTimetableRow(slotId);
    queueAuditEvent("DELETE", "TIMETABLE", slotId);
    return;
  }
  const client = requireDatabaseClient();
