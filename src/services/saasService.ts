...
type SchoolRow = {
  ...
  report_card_template_file_name: string | null;
  report_card_template_updated_at: string | null;
...
const normalizePlatformActionError = (error: unknown) => {
  ...
  if (lowerMessage.includes("duplicate key value") || lowerMessage.includes("unique constraint")) {
    if (lowerMessage.includes("admin_email") || lowerMessage.includes("admin email")) {
      return new Error("This admin email already exists.");
    }
    if (lowerMessage.includes("email")) {
      return new Error("This email already exists.");
    }
    if (lowerMessage.includes("phone")) {
      return new Error("This phone number already exists.");
    }
...
const ensureUniqueSchoolEntry = async (values: Pick<SchoolCreateValues, "name" | "email" | "adminEmail">) => {
  const normalizedName = normalizeLookupValue(values.name);
  const normalizedEmail = normalizeLookupValue(values.email);
  const normalizedAdminEmail = normalizeLookupValue(values.adminEmail);
  const schools = await listSchools();
  const duplicate = schools.find((item) => {
    const schoolEmail = normalizeLookupValue(item.email);
    const adminEmail = normalizeLookupValue(item.adminEmail);
    return (
      normalizeLookupValue(item.name) === normalizedName ||
      (normalizedEmail && schoolEmail === normalizedEmail) ||
      (normalizedAdminEmail && adminEmail === normalizedAdminEmail)
    );
  });
  ...
  if (normalizedAdminEmail && normalizeLookupValue(duplicate.adminEmail) === normalizedAdminEmail) {
    throw new Error("This admin email already exists.");
  }

  if (normalizedEmail && normalizeLookupValue(duplicate.email) === normalizedEmail) {
    throw new Error("This school email already exists.");
  }

  throw new Error(`School "${values.name.trim()}" already exists.`);
};
...
const invokePlatformAction = async <T>(action: string, payload: Record<string, unknown>) => {
  ...
  if (!response.ok || result.error) {
    ...
    if (response.status >= 500) {
      throw normalizePlatformActionError(
        new Error(
    ...
    throw normalizePlatformActionError(
      new Error(result.error ?? `Platform action failed (${response.status} ${response.statusText}).`),
    );
  }
...
const mapSchool = (row: SchoolRow): SchoolRecord => ({
  ...
  reportCardTemplateFileName: row.report_card_template_file_name,
  reportCardTemplateUpdatedAt: row.report_card_template_updated_at,
  createdAt: row.created_at,
});

const mapBoard = (row: BoardRow): BoardRecord => ({
  id: row.id,
  name: row.name,
  state: row.state,
...
export const listBoards = async (): Promise<BoardRecord[]> => {
  const rows = await invokePlatformAction<BoardRow[]>("list_boards", {});
  return rows.map(mapBoard);
};

export const createBoard = async (values: BoardCreateValues): Promise<BoardRecord> => {
  ...
  if (name.toLowerCase() === "state" && !state) {
    throw new Error("Select a state for the State board.");
  }

  const row = await invokePlatformAction<BoardRow>("create_board", {
    name,
    state: state || null,
  });
...
export const createSchool = async (values: SchoolCreateValues) => {
  ...
  if (!values.boardId.trim()) throw new Error("Board selection is required.");
  ...
  await ensureUniqueSchoolEntry(values);
  ...
  return invokePlatformAction<SchoolRecord>("create_school_bundle", {
    ...
    reportCardTemplateFileName: values.reportCardTemplateFileName?.trim() ?? "",
    reportCardTemplateBase64: values.reportCardTemplateBase64?.trim() ?? "",
  });
};
...
export const updateSchool = async (schoolId: string, values: SchoolUpdateValues) => {
  ...
  if (!values.boardId.trim()) {
    throw new Error("Board selection is required.");
  }
...
export const generateReportCards = async (
  values: ReportCardGenerationValues,
): Promise<ReportCardGenerationResult> => {
  ...
  if (values.reportType === "individual" && !values.studentId?.trim()) {
    throw new Error("Student selection is required for an individual report.");
  }

  return invokePlatformAction<ReportCardGenerationResult>("generate_report_cards", {
    reportType: values.reportType,
    examId: values.examId.trim(),
