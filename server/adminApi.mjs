...
const BOOTSTRAP_ACCESS_TOKEN = "inddia-bootstrap-super-admin";
...
const getFirestoreBoolean = (record, keys) => {
  const value = getFirestoreField(record, keys);
  if (value === null || value === undefined || value === "") return null;
...
const getFirestoreUserPayload = (id, data) => ({
  id,
  name: getFirestoreString(data, ["name"]) ?? "GSR ERP User",
...
const resolveFirestoreStaffDocForUser = async (schoolId, user, profile) => {
  if (!firebaseAdminDb || !schoolId) return null;

  const normalizedUserId = String(user?.id ?? "").trim();
  const normalizedEmail = String(user?.email ?? profile?.email ?? "").trim().toLowerCase();
  const normalizedName = String(profile?.name ?? user?.user_metadata?.name ?? "").trim().toLowerCase();
  const schoolScopedDocs = await getFirestoreSchoolScopedDocs("staff", schoolId);
  const staffDocs = schoolScopedDocs.length > 0 ? schoolScopedDocs : await getAllFirestoreDocs("staff");
  const isSchoolMatch = (doc) => {
    const data = getFirestoreDocData(doc);
    const docSchoolId = String(getFirestoreString(data, ["schoolId", "school_id"]) ?? "").trim();
    return !docSchoolId || docSchoolId === schoolId;
  };

  const exactUserMatch = staffDocs.find((doc) => {
    const data = getFirestoreDocData(doc);
    const docUserId = String(getFirestoreString(data, ["userId", "user_id"]) ?? "").trim();
    return (
      isSchoolMatch(doc) &&
      (
        (docUserId && docUserId === normalizedUserId) ||
        (normalizedUserId && doc.id === normalizedUserId)
      )
    );
  });
  ...
  if (normalizedEmail) {
    const directEmailMatch = staffDocs.find((doc) => {
      const data = getFirestoreDocData(doc);
      const docEmail = String(getFirestoreString(data, ["email"]) ?? "").trim().toLowerCase();
      return isSchoolMatch(doc) && docEmail && docEmail === normalizedEmail;
    });
    if (directEmailMatch) {
    ...
    const userIds = Array.from(
      new Set(
        staffDocs
          .filter((doc) => isSchoolMatch(doc))
          .map((doc) => String(getFirestoreString(getFirestoreDocData(doc), ["userId", "user_id"]) ?? "").trim())
          .filter(Boolean),
      ),
    );
    const userSnapshots = await Promise.all(userIds.map((id) => findFirestoreUserById(id)));
    const emailMatch = userSnapshots.find(
      (candidate) => String(candidate?.email ?? "").trim().toLowerCase() === normalizedEmail,
    );
    if (emailMatch) {
      return staffDocs.find((doc) => {
        const data = getFirestoreDocData(doc);
        return (
          isSchoolMatch(doc) &&
          String(getFirestoreString(data, ["userId", "user_id"]) ?? "").trim() === emailMatch.id
        );
      }) ?? null;
    }
  }
  ...
  if (normalizedName) {
    const nameMatch = staffDocs.find((doc) => {
      const data = getFirestoreDocData(doc);
      const docName = String(getFirestoreString(data, ["name"]) ?? "").trim().toLowerCase();
      return isSchoolMatch(doc) && docName && docName === normalizedName;
    });
    if (nameMatch) {
      return nameMatch;
    }
...
const findFirestoreStaffDocByEmail = async (schoolId, email) => {
  if (!firebaseAdminDb || !email) return null;
  const normalizedEmail = String(email).trim().toLowerCase();
  if (!normalizedEmail) return null;

  const staffDocs = schoolId
    ? await getFirestoreSchoolScopedDocs("staff", schoolId)
    : await getAllFirestoreDocs("staff");
...
const upsertFirebaseStaffAccount = async ({
  ...
  try {
    ...
    const existingStaffDoc =
      (await findFirestoreStaffDocByUserId(authUser.user.id)) ||
      (await findFirestoreStaffDocByEmail(tenantSchoolId, email));
...
const normalizeRoleValue = (value) => {
  ...
  if (normalized === "principle" || normalized === "school_principal" || normalized === "head_principal") return "principal";
...
const getAuthUserMetadata = (authUser) => {
  ...
  const name =
    String(userMetadata.name ?? appMetadata.name ?? authUser.email?.split("@")[0] ?? "GSR ERP User").trim() ||
    "GSR ERP User";
  ...
  if (firebaseAdminAuth && hasFirebaseAdminConfig) {
    ...
    while (true) {
      ...
      users.push(...result.users.map((user) => ({
        ...
        user_metadata: {
          ...(user.customClaims ?? {}),
          name: user.displayName ?? user.email?.split("@")[0] ?? "GSR ERP User",
        },
      })));
  ...
  for (const holiday of holidayRows) {
    await googleCalendarRequest(`/calendars/${encodeURIComponent(googleCalendarId)}/events`, {
      ...
      body: JSON.stringify({
        ...
        description: holiday.description || "School holiday managed by GSR ERP",
...
const requireAuthenticatedUser = async (authHeader) => {
  ...
  if (accessToken === BOOTSTRAP_ACCESS_TOKEN) {
    return {
...
const getAuthContext = async (authHeader) => {
  ...
  return {
    user: {
      ...
      name: String(profile?.name ?? user?.email?.split("@")[0] ?? "GSR ERP User").trim() || "GSR ERP User",
...
const requireAdminOrWorkspace = async (authHeader, allowedWorkspaces) => {
  ...
  if (firebaseAdminDb) {
    ...
    const staffDoc = await resolveFirestoreStaffDocForUser(schoolId, user, profile);
    if (!staffDoc) {
      throw new Error("Staff profile not found.");
    }

    const workspace = normalizeStaffWorkspace(getFirestoreString(getFirestoreDocData(staffDoc), ["role"]));
...
const requireAdminPrincipalOrWorkspace = async (authHeader, allowedWorkspaces) => {
  const context = await getUserProfile(authHeader);
  const { user, profile } = context;
...
const saveTeacherAttendance = async (payload, authHeader) => {
  ...
  if (!isAdmin) {
    if (firebaseAdminDb) {
      const staffDoc = await resolveFirestoreStaffDocForUser(tenantSchoolId, user, profile);

      if (!staffDoc) {
        throw new Error("Teacher profile not found.");
      }

      teacherId = staffDoc.id;
      const attendanceDay = getAttendanceWeekday(date);
      const timetableDocs = await getFirestoreSchoolScopedDocs("timetable", tenantSchoolId);
      const matchingAssignments = timetableDocs.filter((doc) => {
        const data = getFirestoreDocData(doc);
        const slotClass = String(getFirestoreString(data, ["className", "class"]) ?? "").trim();
        const slotSection = String(getFirestoreString(data, ["section"]) ?? "").trim();
        const slotTeacherId = String(getFirestoreString(data, ["teacherId", "teacher_id"]) ?? "").trim();
        const slotSubjectId = String(getFirestoreString(data, ["subjectId", "subject_id"]) ?? "").trim();
        const slotDay = String(getFirestoreString(data, ["day"]) ?? "").trim();
        const isBreak = Boolean(getFirestoreBoolean(data, ["isBreak", "is_break"]) ?? false);
        const isCancelled = Boolean(getFirestoreBoolean(data, ["isCancelled", "is_cancelled"]) ?? false);
      ...
      const schoolLatitude = getFirestoreNumber(schoolData, ["attendanceGeoLatitude", "attendance_geo_latitude"]);
      const schoolLongitude = getFirestoreNumber(schoolData, ["attendanceGeoLongitude", "attendance_geo_longitude"]);
      const schoolRadiusMeters = getFirestoreNumber(schoolData, ["attendanceGeoRadiusMeters", "attendance_geo_radius_meters"]);
      const hasGeoFence =
        schoolLatitude !== null &&
  ...
  if (firebaseAdminDb) {
    ...
    const allowedStudentIds = new Set(
      studentDocs
        .filter((doc) => {
          const data = getFirestoreDocData(doc);
          const classValue = normalizeComparableText(getFirestoreString(data, ["className", "class"]));
          const sectionValue = normalizeComparableText(getFirestoreString(data, ["section"]));
          return (
            classValue === normalizeComparableText(className) &&
            sectionValue === normalizeComparableText(section)
          );
        })
        .map((doc) => doc.id),
    );
    ...
    const existingDocs = await getFirestoreSchoolScopedDocs("attendance", tenantSchoolId);
    const existingMap = new Map(
      existingDocs
        .filter((doc) => {
          const data = getFirestoreDocData(doc);
          const sameDate = String(getFirestoreString(data, ["date"]) ?? "").trim() === date;
          const sameSubject = String(getFirestoreString(data, ["subjectId", "subject_id"]) ?? "").trim() === subjectId;
          return sameDate && sameSubject;
        })
        .map((doc) => {
          const data = getFirestoreDocData(doc);
          return [String(getFirestoreString(data, ["studentId", "student_id"]) ?? "").trim(), doc.id];
        }),
    );
    ...
    for (const row of rows) {
      ...
      const existingId = existingMap.get(studentId) || null;
      const attendanceId = existingId || `${tenantSchoolId}_${date}_${subjectId}_${studentId}`;

      await firebaseAdminDb.collection("attendance").doc(attendanceId).set(
        {
          schoolId: tenantSchoolId,
          studentId,
          subjectId,
          ...
          status,
          updatedAt: new Date().toISOString(),
          ...(existingId ? {} : { createdAt: new Date().toISOString() }),
        },
        { merge: true },
      );
    }
...
const saveStaffAttendance = async (payload, authHeader) => {
  ...
  if (firebaseAdminDb) {
    const staffDocs = await getFirestoreSchoolScopedDocs("staff", tenantSchoolId);
    const validStaffIds = new Set(staffDocs.map((doc) => doc.id));
    if (normalizedEntries.some((entry) => !validStaffIds.has(entry.staffId))) {
    ...
    await Promise.all(
      normalizedEntries.map((entry) => {
        const docId = `${tenantSchoolId}_${entry.staffId}_${date}`;
        return firebaseAdminDb.collection("staff_attendance").doc(docId).set(
          {
            schoolId: tenantSchoolId,
...
const listStaffAttendance = async (payload, authHeader) => {
  ...
  let staffId = String(payload.staffId ?? "").trim();
  ...
  if (firebaseAdminDb) {
    const attendanceDocs = await getFirestoreSchoolScopedDocs("staff_attendance", tenantSchoolId);
    let rows = attendanceDocs
      .map((doc) => {
        const data = doc.data() ?? {};
        return {
          id: doc.id,
          staff_id: getFirestoreString(data, ["staffId", "staff_id"]) ?? "",
          attendance_date: getFirestoreString(data, ["attendanceDate", "attendance_date"]) ?? "",
          status: getFirestoreString(data, ["status"]) ?? "Absent",
          check_in_time: getFirestoreString(data, ["checkInTime", "check_in_time"]),
          check_out_time: getFirestoreString(data, ["checkOutTime", "check_out_time"]),
          notes: getFirestoreString(data, ["notes"]),
          marked_by: getFirestoreString(data, ["markedBy", "marked_by"]),
          created_at: getFirestoreString(data, ["createdAt", "created_at"]),
          updated_at: getFirestoreString(data, ["updatedAt", "updated_at"]),
        };
      })
      .filter((row) => row.staff_id && row.attendance_date);

    if (date) {
      rows = rows.filter((row) => row.attendance_date === date);
    }

    if (month && /^\d{4}-\d{2}$/.test(month)) {
      rows = rows.filter((row) => row.attendance_date >= `${month}-01` && row.attendance_date < `${month}-32`);
    }

    if (staffId) {
      rows = rows.filter((row) => row.staff_id === staffId);
    }
    ...
    const staffMap = new Map(
      staffDocs.map((doc) => {
        ...
        return [
          doc.id,
          {
            name: getFirestoreString(data, ["name"]) ?? "Unknown staff",
            role: getFirestoreString(data, ["role"]) ?? "Staff",
          },
        ];
      }),
    ...
    const markedByUsers = await Promise.all(markedByIds.map((id) => findFirestoreUserById(id)));
    const userMap = new Map(markedByUsers.filter(Boolean).map((item) => [item.id, item]));
...
const getDashboardFast = async (authHeader) => {
  ...
  if (firebaseAdminDb) {
    const [studentDocs, staffDocs, classDocs] = await Promise.all([
      getFirestoreSchoolScopedDocs("students", schoolId),
      getFirestoreSchoolScopedDocs("staff", schoolId),
      getFirestoreSchoolScopedDocs("classes", schoolId),
    ]);

    const recentStudents = studentDocs
      .map((doc) => ({ id: doc.id, data: doc.data() ?? {} }))
      .sort((left, right) =>
        String(right.data.createdAt ?? right.data.created_at ?? "").localeCompare(String(left.data.createdAt ?? left.data.created_at ?? "")),
      )
      .slice(0, 3);

    const recentStaff = staffDocs
      .map((doc) => ({ id: doc.id, data: doc.data() ?? {} }))
      .sort((left, right) =>
        String(right.data.createdAt ?? right.data.created_at ?? "").localeCompare(String(left.data.createdAt ?? left.data.created_at ?? "")),
      )
      ...
      stats: [
        {
          ...
          value: String(studentDocs.length),
          detail: "Across all active classes",
          path: "/dashboard/students",
        },
        {
          label: "Total Teachers",
          value: String(staffDocs.length),
          detail: "Teaching faculty and coordinators",
          path: "/dashboard/staff",
        },
        {
          label: "Total Classes",
          value: String(classDocs.length),
        ...
        },
        {
          ...
          value: "₹0",
          detail: "Billing fast summary is still loading from backend modules",
          path: "/dashboard/fees",
        },
      ],
      recentActivity: [
        ...recentStudents.map((item) => ({
          module: "Students",
          owner: getFirestoreString(item.data, ["name"]) ?? "Student",
          status: "Available",
          time: formatDashboardActivityTime(getFirestoreString(item.data, ["createdAt", "created_at"])),
        })),
        ...recentStaff.map((item) => ({
          module: "Staff",
          owner: getFirestoreString(item.data, ["name"]) ?? "Staff",
          status: getFirestoreString(item.data, ["role"]) ?? "Staff",
          time: formatDashboardActivityTime(getFirestoreString(item.data, ["createdAt", "created_at"])),
        })),
      ].slice(0, 6),
    };
  ...
  const { data: studentRows, error: studentError, count: studentCount } = await service
    .from("students")
  ...
  const { data: staffRows, error: staffError, count: staffCount } = await service
    .from("staff")
  ...
  const { data: classRows, error: classError, count: classCount } = await service
    .from("classes")
  ...
  return {
    stats: [
      {
        label: "Total Students",
        value: String(studentCount ?? studentRows?.length ?? 0),
        detail: "Across all active classes",
        path: "/dashboard/students",
      },
      {
        label: "Total Teachers",
        value: String(staffCount ?? staffRows?.length ?? 0),
        detail: "Teaching faculty and coordinators",
        path: "/dashboard/staff",
      },
      {
        label: "Total Classes",
        value: String(classCount ?? classRows?.length ?? 0),
        detail: "Mapped across sections and streams",
        path: "/dashboard/classes",
      },
      {
    ...
    recentActivity: [
      ...((studentRows ?? []).map((row) => ({
        ...
        status: "Added",
        time: formatDashboardActivityTime(row.created_at),
      }))),
      ...((staffRows ?? []).map((row) => ({
        module: "Staff",
        owner: row.name ?? "Staff",
        status: row.role ?? "Staff",
        time: formatDashboardActivityTime(row.created_at),
      }))),
    ].slice(0, 6),
  };
...
const parseFastListCursor = (cursor) => {
  const value = Number.parseInt(String(cursor ?? "").trim(), 10);
  return Number.isFinite(value) && value >= 0 ? value : 0;
};

const getStudentsFast = async (payload, authHeader) => {
  const { profile } = await getUserProfile(authHeader);
  const schoolId = String(profile.school_id ?? "").trim();
  const limitCount = Math.min(Math.max(Number(payload.limit ?? 20) || 20, 1), 50);
  const offset = parseFastListCursor(payload.cursor);
  ...
  if (firebaseAdminDb) {
    const rows = (await getFirestoreSchoolScopedDocs("students", schoolId))
      .map((doc) => ({
        id: doc.id,
        name: getFirestoreString(doc.data() ?? {}, ["name"]) ?? "Student",
        className: getFirestoreString(doc.data() ?? {}, ["className", "class"]),
        section: getFirestoreString(doc.data() ?? {}, ["section"]),
        createdAt: getFirestoreString(doc.data() ?? {}, ["createdAt", "created_at"]),
      }))
      .sort((left, right) => String(left.name).localeCompare(String(right.name)));

    const items = rows.slice(offset, offset + limitCount);
    return {
      items,
      nextCursor: offset + limitCount < rows.length ? String(offset + limitCount) : null,
    };
  }

  const { data, error, count } = await service
    .from("students")
    .select("id, name, class, section, created_at", { count: "exact" })
    .eq("school_id", schoolId)
    .order("name")
  ...
  return {
    ...
    nextCursor: count && offset + limitCount < count ? String(offset + limitCount) : null,
  };
};

const getStaffFast = async (payload, authHeader) => {
  const { profile } = await getUserProfile(authHeader);
  const schoolId = String(profile.school_id ?? "").trim();
  const limitCount = Math.min(Math.max(Number(payload.limit ?? 20) || 20, 1), 50);
  const offset = parseFastListCursor(payload.cursor);
  ...
  if (firebaseAdminDb) {
    const rows = (await getFirestoreSchoolScopedDocs("staff", schoolId))
      .map((doc) => ({
        id: doc.id,
        name: getFirestoreString(doc.data() ?? {}, ["name"]) ?? "Staff",
        role: getFirestoreString(doc.data() ?? {}, ["role"]) ?? "Staff",
        createdAt: getFirestoreString(doc.data() ?? {}, ["createdAt", "created_at"]),
      }))
      .sort((left, right) => String(left.name).localeCompare(String(right.name)));
...
const staffImportRoleMap = new Map(
  ["Teacher", "HR", "Accounts", "Transport", "Admission", "Principal", "Non-Teaching Staff"].map((role) => [
    role.toLowerCase(),
    role,
  ]),
);
...
const normalizeStaffImportRole = (value) => {
  const normalized = toImportText(value)
    .toLowerCase()
    .replace(/[_-]+/g, " ")
    .replace(/[.,/]+/g, " ")
    .replace(/\s+/g, " ")
  ...
  if (normalized === "principle" || normalized.includes("principal")) {
    return "Principal";
  }
  if (normalized.includes("headmaster") || normalized.includes("head mistress") || normalized.includes("headmistress")) {
    return "Principal";
  }
...
const updateClass = async (payload, authHeader) => {
  ...
  if (firebaseAdminDb) {
    ...
    const previousClassName = String(getFirestoreString(previousData, ["className", "class_name"]) ?? "").trim();
    const previousSection = String(getFirestoreString(previousData, ["section"]) ?? "").trim();
    const classChanged =
      previousClassName.length > 0 &&
      previousSection.length > 0 &&
      (previousClassName !== className || previousSection !== section);
    ...
    if (classChanged) {
      ...
      const timetableDocs = await getFirestoreSchoolScopedDocs("timetable", schoolId);
      ...
      await Promise.all([
        ...studentDocs
          .filter((doc) => {
            const data = doc.data() ?? {};
            return (
              String(getFirestoreString(data, ["className", "class"]) ?? "").trim() === previousClassName &&
              String(getFirestoreString(data, ["section"]) ?? "").trim() === previousSection
            );
          })
          .map((doc) =>
            firebaseAdminDb.collection("students").doc(doc.id).set(
              {
                className,
                class: className,
                section,
        ...
        ...timetableDocs
          ...
          .map((doc) =>
            firebaseAdminDb.collection("timetable").doc(doc.id).set(
              {
                className,
                class: className,
                section,
...
const approveApplicant = async (payload, authHeader) => {
  ...
  if (!applicantId) {
    throw new Error("Applicant approval requires an applicant id.");
  }
  ...
  if (firebaseAdminDb) {
    ...
    const applicantDoc = await firebaseAdminDb.collection("applicants").doc(applicantId).get();
    if (!applicantDoc.exists) {
      throw new Error("Applicant not found.");
    }

    const applicantData = applicantDoc.data() ?? {};
    const applicantSchoolId = String(applicantData.schoolId ?? applicantData.school_id ?? "").trim();
    if (applicantSchoolId && applicantSchoolId !== access.schoolId) {
      throw new Error("Applicant not found.");
    }

    if (String(applicantData.status ?? "").toLowerCase() === "approved") {
      throw new Error("Applicant is already approved.");
    }

    await applicantDoc.ref.set(
      {
        status: "Approved",
        updatedAt: new Date().toISOString(),
      },
    ...
    return {
      id: applicantDoc.id,
      name: String(applicantData.studentName ?? applicantData.name ?? "").trim(),
      email: String(applicantData.email ?? "").trim(),
      className: String(applicantData.classApplied ?? applicantData.class ?? "").trim(),
      status: "Approved",
      parentName: applicantData.parentName ?? applicantData.parent_name ?? null,
      parentEmail: applicantData.parentEmail ?? applicantData.parent_email ?? null,
      parentPhone: applicantData.parentPhone ?? applicantData.parent_phone ?? null,
      createdAt: applicantData.createdAt ?? applicantData.created_at ?? null,
    };
  }

  const { data: applicant, error: applicantError } = await service
    .from("applicants")
    .select("id, name, email, class, class_applied, parent_name, parent_email, parent_phone, status")
    .eq("id", applicantId)
    .single();

  if (applicantError || !applicant) {
    const { data: legacyApplicant, error: legacyError } = await service
      .from("applicants")
      .select("id, name, email, class, status, parent_name, parent_email, parent_phone, created_at")
      .eq("id", applicantId)
      .maybeSingle();

    if (legacyError || !legacyApplicant) {
      throw new Error(applicantError?.message ?? legacyError?.message ?? "Applicant not found.");
    }

    if (String(legacyApplicant.status ?? "").toLowerCase() === "approved") {
      throw new Error("Applicant is already approved.");
    }
    ...
    return {
      ...
      className: legacyApplicant.class ?? "",
      status: "Approved",
      parentName: legacyApplicant.parent_name ?? null,
      parentEmail: legacyApplicant.parent_email ?? null,
      parentPhone: legacyApplicant.parent_phone ?? null,
      createdAt: legacyApplicant.created_at ?? null,
    };
  }
  ...
  return {
    ...
    className: applicant.class_applied ?? applicant.class ?? "",
    status: "Approved",
    parentName: applicant.parent_name ?? null,
    parentEmail: applicant.parent_email ?? null,
    parentPhone: applicant.parent_phone ?? null,
    createdAt: null,
  };
};
...
const updateStudentBundle = async (payload, authHeader) => {
  ...
  if (firebaseAdminDb) {
    const studentSnapshot = await firebaseAdminDb.collection("students").doc(id).get();
    if (studentSnapshot.exists) {
      currentStudentIdentifier = String(
        studentSnapshot.data()?.studentCode ?? studentSnapshot.data()?.student_code ?? "",
      ).trim();
    }
  } else {
    const { data: currentStudentRow } = await service
      .from("students")
      .select("student_code")
      .eq("id", id)
      .maybeSingle();
    currentStudentIdentifier = String(currentStudentRow?.student_code ?? "").trim();
  }
  ...
  const { data: existingStudentUser } = await service
    ...
    .eq("id", userId)
    .maybeSingle();
  const requestedStudentIdentifier = String(payload.schoolId ?? "").trim().toUpperCase();
  const currentStudentEmail = currentStudentIdentifier
    ? `${currentStudentIdentifier.toLowerCase()}@students.inddiaerp.local`
    : "";
  const existingStudentEmail = String(existingStudentUser?.email ?? "").trim().toLowerCase();
  let studentIdentifier = currentStudentIdentifier;

  if (!studentIdentifier) {
    studentIdentifier = await resolveStudentIdentifier(
      tenantSchoolId,
      requestedStudentIdentifier,
      existingStudentEmail || currentStudentEmail,
    );
  } else if (requestedStudentIdentifier && requestedStudentIdentifier !== currentStudentIdentifier) {
    studentIdentifier = await resolveStudentIdentifier(
      tenantSchoolId,
      requestedStudentIdentifier,
      existingStudentEmail || currentStudentEmail,
...
const safeTemplateValue = (value) => {
  if (value === null || value === undefined) return "-";
  const stringValue = String(value).trim();
  return stringValue ? stringValue : "-";
};

const formatNumberValue = (value) => (Number.isFinite(value) ? String(value) : "-");
const formatPercentageValue = (value) => (Number.isFinite(value) ? `${Number(value).toFixed(2)}%` : "-");

const normalizeBoardNameValue = (value) =>
  String(value ?? "")
    .trim()
    .replace(/\s+/g, " ")
    .toLowerCase();

const isStateBoard = (value) => {
  const normalized = normalizeBoardNameValue(value);
  return normalized === "state" || normalized === "state board";
};

const mapBoardRecord = (id, row) => {
  const name = getFirestoreString(row, ["name"]) ?? "";
  const state = getFirestoreString(row, ["state"]);
  return {
    id,
    name,
    state,
    displayName: state ? `${name} - ${state}` : name,
    createdAt: getFirestoreString(row, ["createdAt", "created_at"]),
  };
};

const getBoardRecordById = async (boardId) => {
  if (!boardId) return null;

  if (firebaseAdminDb) {
    const snapshot = await firebaseAdminDb.collection("boards").doc(boardId).get();
    if (!snapshot.exists) return null;
    return mapBoardRecord(snapshot.id, snapshot.data() ?? {});
  }

  const { data, error } = await service.from("boards").select(BOARD_SELECT).eq("id", boardId).maybeSingle();
  if (error) throw new Error(error.message);
  if (!data) return null;
  return {
    id: data.id,
    name: data.name,
    state: data.state,
    displayName: data.state ? `${data.name} - ${data.state}` : data.name,
    createdAt: data.created_at,
  };
};

const normalizeReportCardTemplate = (payload) => {
  const fileName = String(payload.reportCardTemplateFileName ?? "").trim();
  const base64 = sanitizeBase64(payload.reportCardTemplateBase64);
  ...
  if (!fileName || !base64) {
    throw new Error("Both the report card template file name and file content are required.");
  }

  if (!fileName.toLowerCase().endsWith(".xlsx")) {
    throw new Error("Report card template must be an .xlsx file.");
  }

  const buffer = Buffer.from(base64, "base64");
  if (!buffer.length) {
    throw new Error("Uploaded report card template is empty.");
  }

  if (buffer.length > 700 * 1024) {
    throw new Error("Report card template is too large. Keep the .xlsx file under 700 KB.");
  }
  ...
  try {
    ...
    throw new Error("Invalid template. Upload a valid .xlsx report card template.");
  }

  if (!workbook.SheetNames?.length) {
    throw new Error("Invalid template. The workbook must contain at least one sheet.");
  }
...
const mapSchoolDirectoryRow = (schoolId, school, admin, subscription, payment) => ({
  ...
  boardId: school?.board_id ?? null,
  boardName: school?.board_name ?? null,
  boardState: school?.board_state ?? null,
  reportCardTemplateFileName: school?.report_card_template_file_name ?? null,
  reportCardTemplateUpdatedAt: school?.report_card_template_updated_at ?? null,
  createdAt: school?.created_at ?? admin?.created_at ?? subscription?.created_at ?? payment?.created_at ?? null,
  adminName: admin?.name ?? null,
  adminEmail: admin?.email ?? null,
});

const findSchoolAdminUserProfile = async (schoolId) => {
  if (!schoolId) return null;

  if (firebaseAdminDb) {
    const docs = await getFirestoreSchoolScopedDocs("users", schoolId);
    const adminDoc = docs.find((doc) => normalizeRoleValue(doc.data()?.role) === "admin");
    ...
    return {
      ...
      email: getFirestoreString(data, ["email"]),
      role: normalizeRoleValue(data.role) ?? "admin",
      school_id: getFirestoreString(data, ["schoolId", "school_id"]) ?? schoolId,
    };
  }
...
const createSchoolBundle = async (payload, context) => {
  ...
  const template = normalizeReportCardTemplate(payload);
  ...
  const board = await getBoardRecordById(boardId);

  if (!name || !adminName || !adminEmail || !adminPassword || !boardId) {
    throw new Error("School, board, and admin details are required.");
  }
  ...
  if (firebaseAdminDb) {
    ...
    try {
      await schoolRef.set({
        ...
        reportCardTemplateFileName: template?.fileName ?? null,
        reportCardTemplateBase64: template?.base64 ?? null,
        reportCardTemplateUpdatedAt: template?.updatedAt ?? null,
        subscriptionStatus: "Trial",
      ...
      return {
        ...
        boardId: getFirestoreString(school, ["boardId", "board_id"]) ?? board.id,
        boardName: getFirestoreString(school, ["boardName", "board_name"]) ?? board.name,
        boardState: getFirestoreString(school, ["boardState", "board_state"]) ?? board.state,
        reportCardTemplateFileName: getFirestoreString(school, ["reportCardTemplateFileName", "report_card_template_file_name"]),
        reportCardTemplateUpdatedAt: getFirestoreString(school, ["reportCardTemplateUpdatedAt", "report_card_template_updated_at"]),
  ...
  const { data: school, error: schoolError } = await service
    ...
    .insert({
      ...
      report_card_template_file_name: template?.fileName ?? null,
      report_card_template_base64: template?.base64 ?? null,
      report_card_template_updated_at: template?.updatedAt ?? null,
      subscription_status: "Trial",
  ...
  return {
    ...
    boardId: school.board_id,
    boardName: school.board_name,
    boardState: school.board_state,
    reportCardTemplateFileName: school.report_card_template_file_name,
    reportCardTemplateUpdatedAt: school.report_card_template_updated_at,
...
const listBoards = async () => {
  if (firebaseAdminDb) {
    const snapshot = await firebaseAdminDb.collection("boards").get();
    return snapshot.docs
      .map((doc) => mapBoardRecord(doc.id, doc.data() ?? {}))
      .sort((left, right) => left.displayName.localeCompare(right.displayName));
  }

  const { data, error } = await service.from("boards").select(BOARD_SELECT).order("name");
  if (error) throw new Error(error.message);
  ...
  return (data ?? [])
    .map((row) => ({
      ...
      name: row.name,
      state: row.state,
      displayName: row.state ? `${row.name} - ${row.state}` : row.name,
      createdAt: row.created_at,
    }))
    .sort((left, right) => left.displayName.localeCompare(right.displayName));
};

const createBoard = async (payload, context) => {
  const name = String(payload.name ?? "").trim();
  ...
  if (isStateBoard(name) && !state) {
    throw new Error("Select a state for the State board.");
  }

  const normalizedName = normalizeBoardNameValue(name);
  const normalizedState = state ? state.toLowerCase() : "";

  const existingBoards = await listBoards();
  const duplicate = existingBoards.find(
    (board) =>
      normalizeBoardNameValue(board.name) === normalizedName &&
      String(board.state ?? "").trim().toLowerCase() === normalizedState,
  );
  ...
  if (firebaseAdminDb) {
    const ref = firebaseAdminDb.collection("boards").doc();
    await ref.set({
      name,
      state,
      createdAt: new Date().toISOString(),
    });
    await logSuperAdminAuditEvent(context, null, "CREATE", "SUPERADMIN_BOARD", ref.id);
    return mapBoardRecord(ref.id, { name, state, createdAt: new Date().toISOString() });
  }
  ...
  if (error || !data) {
    throw new Error(error?.message ?? "Unable to create board.");
  }

  await logSuperAdminAuditEvent(context, null, "CREATE", "SUPERADMIN_BOARD", data.id);
  return {
    id: data.id,
    name: data.name,
    state: data.state,
...
const updateSchool = async (payload, context) => {
  ...
  const address = String(payload.address ?? "").trim() || null;
  const boardId = String(payload.boardId ?? "").trim();
  const adminName = String(payload.adminName ?? "").trim() || null;
  const adminEmail = String(payload.adminEmail ?? "").trim().toLowerCase() || null;
  const adminPassword = String(payload.adminPassword ?? "");
  const geoConfig = await resolveAttendanceGeoConfig(payload.attendanceMapLink, payload.attendanceGeoRadiusMeters);
  const template = normalizeReportCardTemplate(payload);
  ...
  if (!schoolId || !name || !boardId) {
    throw new Error("School id, school name, and board are required.");
  }
  ...
  const adminProfile = await findSchoolAdminUserProfile(schoolId);
  if (adminEmail && !adminProfile) {
    throw new Error("School admin account was not found for this tenant.");
  }
  ...
  if (firebaseAdminDb) {
    const schoolRef = firebaseAdminDb.collection("schools").doc(schoolId);
    const schoolSnapshot = await schoolRef.get();
    ...
    await schoolRef.set(
      {
        ...
        ...(template
          ? {
              reportCardTemplateFileName: template.fileName,
              reportCardTemplateBase64: template.base64,
              reportCardTemplateUpdatedAt: template.updatedAt,
            }
          : {}),
      },
      { merge: true },
    ...
    if (adminProfile && (adminName || adminEmail || adminPassword)) {
      const nextAdminEmail = adminEmail ?? (String(adminProfile.email ?? "").trim().toLowerCase() || null);
      if (!nextAdminEmail) {
        throw new Error("Admin email is required to update the school admin account.");
      }

      await updateManagedAuthUser(adminProfile.id, {
        email: nextAdminEmail,
        password: adminPassword || undefined,
        name: adminName ?? (String(adminProfile.name ?? "").trim() || name),
        role: "admin",
        schoolId,
      });

      await updatePublicUserProfile(adminProfile.id, {
        name: adminName ?? (String(adminProfile.name ?? "").trim() || name),
        email: nextAdminEmail,
        role: "admin",
        school_id: schoolId,
    ...
    const updatedSnapshot = await schoolRef.get();
    const updatedSchool = updatedSnapshot.data() ?? {};

    return {
      id: schoolId,
      name: getFirestoreString(updatedSchool, ["name"]) ?? name,
      email: getFirestoreString(updatedSchool, ["email"]),
      phone: getFirestoreString(updatedSchool, ["phone"]),
      address: getFirestoreString(updatedSchool, ["address"]),
      attendanceMapLink: getFirestoreString(updatedSchool, ["attendanceMapLink", "attendance_map_link"]),
      attendanceGeoLatitude: getFirestoreNumber(updatedSchool, ["attendanceGeoLatitude", "attendance_geo_latitude"]),
      attendanceGeoLongitude: getFirestoreNumber(updatedSchool, ["attendanceGeoLongitude", "attendance_geo_longitude"]),
      attendanceGeoRadiusMeters: getFirestoreNumber(updatedSchool, ["attendanceGeoRadiusMeters", "attendance_geo_radius_meters"]),
      billingContactName: getFirestoreString(updatedSchool, ["billingContactName", "billing_contact_name"]),
      billingContactEmail: getFirestoreString(updatedSchool, ["billingContactEmail", "billing_contact_email"]),
      billingContactPhone: getFirestoreString(updatedSchool, ["billingContactPhone", "billing_contact_phone"]),
      billingAddress: getFirestoreString(updatedSchool, ["billingAddress", "billing_address"]),
      financeEmail: getFirestoreString(updatedSchool, ["financeEmail", "finance_email"]),
      taxId: getFirestoreString(updatedSchool, ["taxId", "tax_id"]),
      authorizedSignatory: getFirestoreString(updatedSchool, ["authorizedSignatory", "authorized_signatory"]),
      logoUrl: getFirestoreString(updatedSchool, ["logoUrl", "logo_url"]),
      themeColor: getFirestoreString(updatedSchool, ["themeColor", "theme_color"]),
      subscriptionStatus: getFirestoreString(updatedSchool, ["subscriptionStatus", "subscription_status"]) ?? subscriptionStatus,
      subscriptionPlan: getFirestoreString(updatedSchool, ["subscriptionPlan", "subscription_plan"]) ?? subscriptionPlan,
      storageLimit: getFirestoreNumber(updatedSchool, ["storageLimit", "storage_limit"]),
      studentLimit: getFirestoreNumber(updatedSchool, ["studentLimit", "student_limit"]),
      staffLimit: getFirestoreNumber(updatedSchool, ["staffLimit", "staff_limit"]),
      renewalNoticeDays: getFirestoreNumber(updatedSchool, ["renewalNoticeDays", "renewal_notice_days"]),
      expiryDate: getFirestoreString(updatedSchool, ["expiryDate", "expiry_date"]),
      boardId: getFirestoreString(updatedSchool, ["boardId", "board_id"]) ?? board.id,
      boardName: getFirestoreString(updatedSchool, ["boardName", "board_name"]) ?? board.name,
      boardState: getFirestoreString(updatedSchool, ["boardState", "board_state"]) ?? board.state,
      reportCardTemplateFileName: getFirestoreString(updatedSchool, ["reportCardTemplateFileName", "report_card_template_file_name"]),
      reportCardTemplateUpdatedAt: getFirestoreString(updatedSchool, ["reportCardTemplateUpdatedAt", "report_card_template_updated_at"]),
      createdAt: getFirestoreString(updatedSchool, ["createdAt", "created_at"]),
      adminName: adminName ?? adminProfile?.name ?? null,
      adminEmail: adminEmail ?? adminProfile?.email ?? null,
    };
  }
  ...
  const { data, error } = await service
    ...
    .update({
      ...
      ...(template
        ? {
            report_card_template_file_name: template.fileName,
            report_card_template_base64: template.base64,
            report_card_template_updated_at: template.updatedAt,
          }
        : {}),
    })
    .eq("id", schoolId)
  ...
  const result = {
    ...
    reportCardTemplateFileName: data.report_card_template_file_name,
    reportCardTemplateUpdatedAt: data.report_card_template_updated_at,
    createdAt: data.created_at,
    adminName: adminName ?? adminProfile?.name ?? null,
    adminEmail: adminEmail ?? adminProfile?.email ?? null,
  };
...
const requireReportCardAccess = async (authHeader) => {
  const context = await getUserProfile(authHeader);
  const role = String(context.profile.role ?? "").trim().toLowerCase();
  const schoolId = String(context.profile.school_id ?? "").trim();

  if (!schoolId) {
    throw new Error("School context is missing for this account.");
  }

  if (role === "principal") {
    return { ...context, schoolId, access: "principal", assignedClass: null, assignedSection: null };
  }

  if (role !== "staff") {
    throw new Error("Only principals and class coordinators can access report cards.");
  }

  if (firebaseAdminDb) {
    const staffDoc = await resolveFirestoreStaffDocForUser(schoolId, context.user, context.profile);
    if (!staffDoc) {
      throw new Error("Staff profile not found.");
    }

    const staffData = getFirestoreDocData(staffDoc);
    const isClassCoordinator = Boolean(getFirestoreField(staffData, ["isClassCoordinator", "is_class_coordinator"]));
    ...
    return {
      ...
      schoolId,
      access: "coordinator",
      assignedClass: getFirestoreString(staffData, ["assignedClass", "assigned_class"]),
      assignedSection: getFirestoreString(staffData, ["assignedSection", "assigned_section"]),
    };
  }
  ...
  return {
    ...
    schoolId,
    access: "coordinator",
    assignedClass: staff.assigned_class ?? null,
    assignedSection: staff.assigned_section ?? null,
  };
};

const getSchoolReportCardConfig = async (schoolId) => {
  if (firebaseAdminDb) {
    const snapshot = await firebaseAdminDb.collection("schools").doc(schoolId).get();
    if (!snapshot.exists) {
      throw new Error("School not found.");
    }
    ...
    const data = snapshot.data() ?? {};
    return {
      name: getFirestoreString(data, ["name"]) ?? "School",
      boardName: getFirestoreString(data, ["boardName", "board_name"]),
      boardState: getFirestoreString(data, ["boardState", "board_state"]),
      templateFileName: getFirestoreString(data, ["reportCardTemplateFileName", "report_card_template_file_name"]),
      templateBase64: getFirestoreString(data, ["reportCardTemplateBase64", "report_card_template_base64"]),
    };
  }

  const { data, error } = await service
    .from("schools")
    .select("name, board_name, board_state, report_card_template_file_name, report_card_template_base64")
    .eq("id", schoolId)
    .single();
  ...
  return {
    ...
    boardName: data.board_name,
    boardState: data.board_state,
    templateFileName: data.report_card_template_file_name,
    templateBase64: data.report_card_template_base64,
  };
};

const getExamReportSource = async (schoolId, examId, className, section) => {
  if (firebaseAdminDb) {
    const [examSnapshot, subjectDocs, studentDocs, examSubjectDocs, markDocs] = await Promise.all([
      firebaseAdminDb.collection("exams").doc(examId).get(),
      getFirestoreSchoolScopedDocs("subjects", schoolId),
      getFirestoreSchoolScopedDocs("students", schoolId),
      getFirestoreSchoolScopedDocs("exam_subjects", schoolId),
      getFirestoreSchoolScopedDocs("marks", schoolId),
    ]);
    ...
    const examClass = getFirestoreString(exam, ["className", "class"]) ?? "";
    const examSection = getFirestoreString(exam, ["section"]) ?? "";

    if (examClass !== className || String(examSection).trim() !== String(section).trim()) {
      throw new Error("Selected exam does not belong to the chosen class and section.");
    }

    return {
      exam: {
        id: examId,
        name: getFirestoreString(exam, ["name"]) ?? "Exam",
        className: examClass,
        section: examSection,
      },
      subjects: new Map(subjectDocs.map((doc) => [doc.id, getFirestoreString(doc.data, ["name"]) ?? "Subject"])),
      students: studentDocs
        .map((doc) => ({
          id: doc.id,
          name: getFirestoreString(doc.data, ["name"]) ?? "Student",
          studentCode: getFirestoreString(doc.data, ["studentCode", "student_code"]),
          className: getFirestoreString(doc.data, ["className", "class"]) ?? "",
          section: getFirestoreString(doc.data, ["section"]) ?? "",
        }))
        .filter((student) => student.className === className && student.section === section),
      examSubjects: examSubjectDocs
        .map((doc) => ({
          examId: getFirestoreString(doc.data, ["examId", "exam_id"]),
          subjectId: getFirestoreString(doc.data, ["subjectId", "subject_id"]),
          maxMarks: getFirestoreNumber(doc.data, ["maxMarks", "max_marks"]),
        }))
        .filter((row) => row.examId === examId && row.subjectId),
      marks: markDocs
        .map((doc) => ({
          examId: getFirestoreString(doc.data, ["examId", "exam_id"]),
          studentId: getFirestoreString(doc.data, ["studentId", "student_id"]),
          subjectId: getFirestoreString(doc.data, ["subjectId", "subject_id"]),
          marksObtained: getFirestoreNumber(doc.data, ["marksObtained", "marks_obtained"]),
          grade: getFirestoreString(doc.data, ["grade"]),
        }))
        .filter((row) => row.examId === examId && row.studentId && row.subjectId),
    };
  }

  const [{ data: exam, error: examError }, { data: subjects, error: subjectsError }, { data: students, error: studentsError }, { data: examSubjects, error: examSubjectsError }, { data: marks, error: marksError }] =
    await Promise.all([
      service.from("exams").select("id, name, class, section").eq("id", examId).single(),
      service.from("subjects").select("id, name").eq("school_id", schoolId),
      service.from("students").select("id, name, student_code, class, section").eq("school_id", schoolId),
      service.from("exam_subjects").select("exam_id, subject_id, max_marks").eq("exam_id", examId),
      service.from("marks").select("exam_id, student_id, subject_id, marks_obtained, grade").eq("exam_id", examId),
    ]);
  ...
  if (String(exam.class ?? "") !== className || String(exam.section ?? "").trim() !== String(section).trim()) {
    throw new Error("Selected exam does not belong to the chosen class and section.");
  }
  ...
  return {
    ...
    subjects: new Map((subjects ?? []).map((row) => [row.id, row.name ?? "Subject"])),
    students: (students ?? [])
      .map((row) => ({
        id: row.id,
        name: row.name ?? "Student",
...
const buildStudentReportData = ({ schoolName, boardLabel, examName, className, section, student, examSubjects, subjectMap, markRows }) => {
  const marksBySubjectId = new Map(markRows.map((row) => [row.subjectId, row]));
  const subjects = examSubjects.map((subjectRow) => {
    const mark = marksBySubjectId.get(subjectRow.subjectId);
    const hasMarks = Number.isFinite(mark?.marksObtained);
    const marksObtained = hasMarks ? Number(mark.marksObtained) : null;
    const maxMarks = Number.isFinite(subjectRow.maxMarks) ? Number(subjectRow.maxMarks) : null;
    const percentage = hasMarks && Number.isFinite(maxMarks) && maxMarks > 0 ? (marksObtained / maxMarks) * 100 : null;
    const grade = hasMarks
      ? String(mark?.grade ?? "").trim() || (percentage != null ? (percentage >= 90 ? "A" : percentage >= 75 ? "B" : percentage >= 50 ? "C" : "F") : null)
      : null;

    return {
      subjectName: subjectMap.get(subjectRow.subjectId) ?? "Subject",
      marksObtained,
      maxMarks,
      percentage,
      grade,
  ...
  const hasCompleteMarks = subjects.length > 0 && subjects.every((subject) => subject.hasMarks && Number.isFinite(subject.maxMarks));
  const totalMarks = hasCompleteMarks ? subjects.reduce((sum, subject) => sum + Number(subject.marksObtained ?? 0), 0) : null;
  const totalMaxMarks = hasCompleteMarks ? subjects.reduce((sum, subject) => sum + Number(subject.maxMarks ?? 0), 0) : null;
  const overallPercentage = hasCompleteMarks && totalMaxMarks ? (totalMarks / totalMaxMarks) * 100 : null;
  const finalGrade =
    overallPercentage == null ? null : overallPercentage >= 90 ? "A" : overallPercentage >= 75 ? "B" : overallPercentage >= 50 ? "C" : "F";

  const subjectRowsText =
    subjects.length > 0
      ? subjects
          .map(
            (subject, index) =>
              `${index + 1}. ${subject.subjectName} | ${safeTemplateValue(subject.marksObtained)} | ${safeTemplateValue(subject.maxMarks)} | ${safeTemplateValue(subject.grade)}`,
          )
          .join("\n")
      : "-";
...
const replaceTemplatePlaceholders = (value, placeholderMap) =>
  String(value ?? "").replace(/\{\{\s*([a-z0-9_]+)\s*\}\}/gi, (_match, key) => placeholderMap[String(key).toLowerCase()] ?? "-");

const createWorkbookFromTemplate = (templateBase64, reports) => {
  const templateBuffer = Buffer.from(sanitizeBase64(templateBase64), "base64");
  const templateWorkbook = XLSX.read(templateBuffer, { type: "buffer" });

  if (!templateWorkbook.SheetNames?.length) {
    throw new Error("Invalid template. The workbook must contain at least one sheet.");
  }
  ...
  reports.forEach((report, index) => {
    const sheet = JSON.parse(JSON.stringify(templateSheet));
    const range = XLSX.utils.decode_range(sheet["!ref"] ?? "A1");
    const placeholderMap = {
      school_name: safeTemplateValue(report.schoolName),
      board_name: safeTemplateValue(report.boardLabel),
      student_name: safeTemplateValue(report.studentName),
      student_code: safeTemplateValue(report.studentCode),
      class: safeTemplateValue(report.className),
      section: safeTemplateValue(report.section),
      exam_name: safeTemplateValue(report.examName),
      total_marks: formatNumberValue(report.totalMarks),
      max_marks: formatNumberValue(report.totalMaxMarks),
      percentage: formatPercentageValue(report.overallPercentage),
      grade: safeTemplateValue(report.finalGrade),
      subject_rows: safeTemplateValue(report.subjectRowsText),
      subject_count: String(report.subjects.length || 0),
    };
    ...
    for (let row = range.s.r; row <= range.e.r; row += 1) {
      for (let col = range.s.c; col <= range.e.c; col += 1) {
        ...
        cell.v = replaceTemplatePlaceholders(cell.v, placeholderMap);
        cell.w = cell.v;
      }
    }

    const sheetName = `${index + 1}-${slugifyFilePart(report.studentName)}`.slice(0, 31) || `student-${index + 1}`;
...
const createReportCardPdf = (reports) => {
  ...
  reports.forEach((report, index) => {
    ...
    let y = 48;
    pdf.setFontSize(18);
    pdf.text(report.schoolName, 40, y);
    y += 22;
    pdf.setFontSize(11);
    pdf.text(`Board: ${safeTemplateValue(report.boardLabel)}`, 40, y);
    y += 16;
    pdf.text(`Exam: ${safeTemplateValue(report.examName)}`, 40, y);
    y += 16;
    pdf.text(`Student: ${safeTemplateValue(report.studentName)} (${safeTemplateValue(report.studentCode)})`, 40, y);
    y += 16;
    pdf.text(`Class: ${safeTemplateValue(report.className)}   Section: ${safeTemplateValue(report.section)}`, 40, y);
    y += 24;

    pdf.setFontSize(12);
    pdf.text("Subjects", 40, y);
    y += 18;
    pdf.setFontSize(10);

    const subjectLines = report.subjects.length
      ? report.subjects.map(
          (subject, subjectIndex) =>
            `${subjectIndex + 1}. ${subject.subjectName} | Marks: ${safeTemplateValue(subject.marksObtained)} | Max: ${safeTemplateValue(subject.maxMarks)} | Grade: ${safeTemplateValue(subject.grade)}`,
        )
      : ["-"];
    ...
    y += 10;
    pdf.setFontSize(11);
    pdf.text(`Total: ${formatNumberValue(report.totalMarks)} / ${formatNumberValue(report.totalMaxMarks)}`, 40, y);
    y += 16;
    pdf.text(`Percentage: ${formatPercentageValue(report.overallPercentage)}`, 40, y);
    y += 16;
    pdf.text(`Grade: ${safeTemplateValue(report.finalGrade)}`, 40, y);
  });

  return Buffer.from(pdf.output("arraybuffer")).toString("base64");
};

const generateReportCards = async (payload, authHeader) => {
  const context = await requireReportCardAccess(authHeader);
  ...
  if (!["individual", "class-wise"].includes(reportType)) {
    throw new Error("Select a valid report card type.");
  }
  if (!examId || !className || !section) {
    throw new Error("Exam, class, and section are required.");
  }
  if (reportType === "individual" && !studentId) {
    throw new Error("Student selection is required for an individual report.");
  }

  if (context.access === "coordinator") {
    if (
      String(context.assignedClass ?? "").trim() !== className ||
      String(context.assignedSection ?? "").trim() !== section
    ) {
      throw new Error("Class coordinators can generate report cards only for their assigned class and section.");
    }
  }

  const schoolConfig = await getSchoolReportCardConfig(context.schoolId);
  if (!schoolConfig.templateBase64 || !schoolConfig.templateFileName) {
    throw new Error("No report card template is uploaded for this school.");
  }

  const source = await getExamReportSource(context.schoolId, examId, className, section);
  let students = source.students;

  if (reportType === "individual") {
    students = students.filter((student) => student.id === studentId);
    if (students.length === 0) {
      throw new Error("Selected student was not found in the chosen class and section.");
    }
  }

  if (students.length === 0) {
    throw new Error("No students were found for the selected class and section.");
  }

  const boardLabel = schoolConfig.boardState
    ? `${safeTemplateValue(schoolConfig.boardName)} - ${safeTemplateValue(schoolConfig.boardState)}`
    : safeTemplateValue(schoolConfig.boardName);
  ...
  const reports = students.map((student) =>
    buildStudentReportData({
      ...
      section,
      student,
      examSubjects: source.examSubjects,
      subjectMap: source.subjects,
      markRows: source.marks.filter((mark) => mark.studentId === student.id),
    }),
  );

  const workbook = createWorkbookFromTemplate(schoolConfig.templateBase64, reports);
  const excelBase64 = Buffer.from(XLSX.write(workbook, { bookType: "xlsx", type: "buffer" })).toString("base64");
  const pdfBase64 = createReportCardPdf(reports);
  const fileBaseName = `${slugifyFilePart(source.exam.name)}-${slugifyFilePart(className)}-${slugifyFilePart(section)}-${reportType}`;
  ...
  return {
    ...
    reportType,
    warnings: source.examSubjects.length === 0 ? ["Exam schedule is missing. Missing values were replaced with '-' in the generated report card."] : [],
  };
};
  ...
  try {
    ...
    switch (action) {
      ...
      case "sync_google_calendar_full":
        ...
        return true;
      case "get_dashboard_fast":
      case "get_students_fast":
      case "get_staff_fast":
      ...
      case "approve_applicant":
        await requireAdminPrincipalOrWorkspace(authHeader, ["admission"]);
        break;
      ...
      case "list_school_plan_change_requests":
        ...
        break;
      case "generate_report_cards":
        await requireReportCardAccess(authHeader);
        break;
      default:
        await requireAdmin(authHeader);
        break;
    ...
    switch (action) {
      ...
      case "get_auth_context":
        ...
        return true;
      case "get_dashboard_fast":
        send(res, 200, { data: await getDashboardFast(authHeader) });
        return true;
      case "get_students_fast":
        send(res, 200, { data: await getStudentsFast(payload, authHeader) });
        return true;
      case "get_staff_fast":
        send(res, 200, { data: await getStaffFast(payload, authHeader) });
        return true;
      case "list_firestore_collection":
      ...
      case "list_schools":
        ...
        return true;
      case "list_boards":
        send(res, 200, { data: await listBoards() });
        return true;
      case "create_board":
        send(res, 200, { data: await createBoard(payload, await requireSuperAdmin(authHeader)) });
        return true;
      case "list_subscriptions":
      ...
      case "list_school_plan_change_requests":
        ...
        return true;
      case "generate_report_cards":
        send(res, 200, { data: await generateReportCards(payload, authHeader) });
        return true;
      case "list_superadmin_payment_requests":
